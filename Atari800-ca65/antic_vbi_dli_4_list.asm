...
Main file   : antic_vbi_dli_4.asm
Current file: antic_vbi_dli_4.asm

000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; Ovládání PMG joystickem v horizontálním směru.
000000r 1               ; Přepis předchozího příkladu do formy založené na makrech.
000000r 1               ;
...
000000r 1               
000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; makro pro vyplnění obrazovky znaky
000000r 1               ; ---------------------------------------------------------------------
000000r 1               .macro  fill_screen screen
000000r 1                       .local clear            ; definice lokálního symbolu
000000r 1                       ldy #0                  ; počitadlo zápisů
000000r 1               clear:
000000r 1                       tya                     ; kód vypisovaného znaku
000000r 1                       sta screen, y           ; tisk znaku na zvolené místo na obrazovce
000000r 1                       sta screen+40*8, y
000000r 1                       sta screen+40*16, y
000000r 1                       iny                     ; zvětšit hodnotu počitadla a offsetu
000000r 1                       cpy #$80
000000r 1                       bne clear               ; skok
000000r 1               .endmacro
000000r 1               
000000r 1               
000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; makro pro nastavení všech spritů
000000r 1               ; ---------------------------------------------------------------------
000000r 1               .macro  setup_sprites sprite_address
...
000000r 1               
000000r 1                       lda #HUE_YELLOW<<4 + 12 ; barva druhého hráče (odstín+intenzita)
...
000000r 1               
000000r 1                       lda #1                   ; priorita hráčů a pozadí
...
000000r 1               
000000r 1                       .local addr             ; definice lokálního symbolu
000000r 1                       addr = 152*256
000000r 1               
000000r 1                       .local next_line        ; definice lokálního symbolu
000000r 1                       ldx #8                  ; začneme na hodnotě o 1 vyšší
000000r 1               next_line:
000000r 1                       lda sprite_address-1, x          ; načíst
000000r 1                       sta addr+PLAYER_0_OFFSET+100, x  ; uložit byte - první hráč
000000r 1                       sta addr+PLAYER_1_OFFSET+100, x  ; uložit byte - druhý hráč
000000r 1                       sta addr+PLAYER_2_OFFSET+100, x  ; uložit byte - třetí hráč
000000r 1                       sta addr+PLAYER_3_OFFSET+100, x  ; uložit byte - čtvrtý hráč
...
000000r 1               
000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; makro pro nastavení DLI i VBI
000000r 1               ; ---------------------------------------------------------------------
000000r 1               .macro  enable_interrupts
000000r 1                       .local NMIEN_DLI        ; definice lokálního symbolu
000000r 1                       .local NMIEN_VBI        ; definice lokálního symbolu
000000r 1                       NMIEN_DLI=$80           ; maska povolení DLI
000000r 1                       NMIEN_VBI=$40           ; maska povolení VBI
000000r 1                       lda #NMIEN_DLI | NMIEN_VBI ; povolení DLI
000000r 1                       sta NMIEN
000000r 1               .endmacro
000000r 1               
000000r 1               
...
000000r 1                       lda #7                  ; změna vektoru pro odložené VBI
000000r 1                       ldx #>handler
000000r 1                       ldy #<handler
000000r 1                       jsr SETVBV              ; zavolat službu systému pro nastavení vektoru
000000r 1               .endmacro
000000r 1               
000000r 1               
000000r 1               ; ---------------------------------------------------------------------
...
000008r 1  31 02        
00000Ar 1  A0 00 98 99          fill_screen screen      ; výpis znaků na obrazovku
00000Er 1  rr rr 99 rr  
000012r 1  rr 99 rr rr  
000016r 1  C8 C0 80 D0  
00001Ar 1  F1           
00001Br 1  A9 50 8D 00          setup_sprites sprite    ; nastavení spritů
00001Fr 1  D0 A9 64 8D  
000023r 1  01 D0 A9 78  
000027r 1  8D 02 D0 A9  
00002Br 1  8C 8D 03 D0  
00002Fr 1  A9 CC 8D C0  
000033r 1  02 A9 EC 8D  
000037r 1  C1 02 A9 5C  
00003Br 1  8D C2 02 A9  
00003Fr 1  9C 8D C3 02  
000043r 1  A9 FF 8D 0D  
000047r 1  D0 8D 0E D0  
00004Br 1  8D 0F D0 8D  
00004Fr 1  10 D0 A9 03  
000053r 1  8D 1D D0 A9  
000057r 1  01 8D 6F 02  
00005Br 1  A9 98 8D 07  
00005Fr 1  D4 A2 08 BD  
000063r 1  rr rr 9D 64  
000067r 1  9A 9D E4 9A  
00006Br 1  9D 64 9B 9D  
00006Fr 1  E4 9B CA D0  
000073r 1  EE A9 2E 8D  
000077r 1  2F 02        
000079r 1  A9 rr 8D 00          set_dli_handler dli     ; nastavení obsluhy DLI
00007Dr 1  02 A9 rr 8D  
000081r 1  01 02        
000083r 1  A9 07 A2 rr          set_vbi_handler vbi     ; nastavení obsluhy VBI
000087r 1  A0 rr 20 5C  
00008Br 1  E4           
00008Cr 1  A9 C0 8D 0E          enable_interrupts       ; povolení přerušení
000090r 1  D4           
000091r 1  4C rr rr             game_loop               ; hlavní herní smyčka
000094r 1               .endproc
000094r 1               
000094r 1               
000094r 1               
000094r 1               ; ---------------------------------------------------------------------
000094r 1               ; subrutina pro obsluhu VBI
000094r 1               ; ---------------------------------------------------------------------
000094r 1               .proc   vbi
000094r 1  AE rr rr             ldx x_pos               ; původní pozice prvního hráče
000097r 1               
000097r 1  AD 78 02             lda STICK0              ; čtení joysticku
00009Ar 1  C9 0B                cmp #11                 ; je nakloněn doleva?
00009Cr 1  D0 01                bne not_left
00009Er 1  CA                   dex                     ; posun hráče doleva
00009Fr 1               not_left:
00009Fr 1  C9 07                cmp #7                  ; je nakloněn doprava?
0000A1r 1  D0 01                bne not_right
0000A3r 1  E8                   inx                     ; posun hráče doprava
0000A4r 1               not_right:
0000A4r 1  8E 00 D0             stx HPOSP0              ; změna pozice prvního hráče
0000A7r 1  8E rr rr             stx x_pos               ; zapamatovat si pozici prvního hráče
0000AAr 1  4C 62 E4             jmp XITVBV              ; zpracovat zbytek odloženého VBLANKu
0000ADr 1               .endproc
0000ADr 1               
0000ADr 1               
0000ADr 1               ; ---------------------------------------------------------------------
0000ADr 1               ; obsluha DLI
0000ADr 1               ; ---------------------------------------------------------------------
0000ADr 1               dli:
0000ADr 1  48                   pha                     ; uschovat akumulátor
0000AEr 1  AD rr rr             lda color               ; barva pozadí
0000B1r 1  8D 18 D0             sta COLPF2              ; přímo nastavit zápisem do HW registru
0000B4r 1  49 80                eor #%10000000          ; negovat nejvyšší bit
0000B6r 1  8D rr rr             sta color
0000B9r 1  68                   pla                     ; obnovit akumulátor
0000BAr 1  40                   rti                     ; návrat z DLI
0000BBr 1               
0000BBr 1               
0000BBr 1               dlist:
0000BBr 1  70 70 70     .byte DL_BLK8, DL_BLK8, DL_BLK8 ; 3x8=24 prázdných obrazových řádků
0000BEr 1  42           .byte DL_LMS+DL_CHR40x8x1       ; určení počáteční adresy obrazové paměti + jeden řádek režimu 2 (GR.0)
0000BFr 1  rr rr        .byte <screen, >screen          ; počáteční adresa obrazové paměti
0000C1r 1  02 02 02 02  .res 6, DL_CHR40x8x1            ; opakovat řádky textového režimu 2 (GR.0)
0000C5r 1  02 02        
0000C7r 1  82           .byte DL_DLI+DL_CHR40x8x1       ; GR.0 ovšem navíc s povolením DLI
0000C8r 1  02 02 02 02  .res 7, DL_CHR40x8x1            ; opakovat řádky textového režimu 2 (GR.0)
0000CCr 1  02 02 02     
0000CFr 1  82           .byte DL_DLI+DL_CHR40x8x1       ; GR.0 ovšem navíc s povolením DLI
0000D0r 1  02 02 02 02  .res 8, DL_CHR40x8x1            ; opakovat řádky textového režimu 2 (GR.0)
0000D4r 1  02 02 02 02  
0000D8r 1  41 rr rr     .byte DL_JVB, <dlist, >dlist    ; skok na začátek display listu
0000DBr 1               
0000DBr 1               
0000DBr 1               color:
0000DBr 1  C4           .byte $c4                       ; původní barva
0000DCr 1               
0000DCr 1               
0000DCr 1               ; data
0000DCr 1  18 3C 7E DB  sprite:   .byte 24, 60, 126, 219, 255, 36, 90, 165
0000E0r 1  FF 24 5A A5  
0000E4r 1               
0000E4r 1               ; horizontální pozice hráče
0000E4r 1  50           x_pos:    .byte 80
0000E5r 1               
0000E5r 1               end:
0000E5r 1               
0000E5r 1               
