...
Main file   : cio_read_file_2.asm
Current file: cio_read_file_2.asm
...
000000r 1               .CODE
000000r 1               
000000r 1               IOCB0 = $00              ; offset odvozený od čísla IOCB kanálu (nutno násobit šestnácti)
...
000000r 1               ; ---------------------------------------------------------------------
000000r 1               .proc main
000000r 1  20 rr rr             jsr CLOSE_CHANNEL_4
000003r 1  20 rr rr             jsr OPEN_CHANNEL_4_AS_H
000006r 1  20 rr rr             jsr READ_MESSAGE_FROM_CHANNEL_4
000009r 1  20 rr rr     	jsr WRITE_MESSAGE_INTO_CHANNEL_0
00000Cr 1  20 rr rr             jsr CLOSE_CHANNEL_4
00000Fr 1               loop:
00000Fr 1  4C rr rr             jmp loop                ; program vlastně nice nedělá - jen cyklí!
000012r 1               .endproc
000012r 1               
000012r 1               
000012r 1               
000012r 1               ; ---------------------------------------------------------------------
000012r 1               ; uzavření kanálu číslo 4
000012r 1               ; ---------------------------------------------------------------------
000012r 1               .proc CLOSE_CHANNEL_4
000012r 1  A2 40                ldx #IOCB4              ; offset odvozený od čísla IOCB kanálu
000014r 1  A9 0C                lda #CLOSE              ; prováděná operace
000016r 1  9D 42 03             sta ICCOM, x            ; zápis prováděné operace do bloku IOCB
000019r 1  20 56 E4             jsr CIOV                ; zavolání rutiny pro CIO
00001Cr 1  60                   rts                     ; návrat ze subrutiny
00001Dr 1               .endproc
00001Dr 1               
00001Dr 1               
00001Dr 1               
00001Dr 1               ; ---------------------------------------------------------------------
00001Dr 1               ; otevření kanálu číslo 4 v režimu zápisu na zařízení H:test.txt
00001Dr 1               ; ---------------------------------------------------------------------
00001Dr 1               .proc OPEN_CHANNEL_4_AS_H
00001Dr 1  A2 40                ldx #IOCB4              ; offset odvozený od čísla IOCB kanálu
00001Fr 1  A9 03                lda #OPEN               ; prováděná operace
000021r 1  9D 42 03             sta ICCOM, x            ; zápis prováděné operace do bloku IOCB
000024r 1               
000024r 1                       ; specifikace jména zařízení + jména souboru (16bitový ukazatel)
000024r 1  A9 rr                lda #<device_name
000026r 1  9D 44 03             sta ICBAL, x
000029r 1  A9 rr                lda #>device_name
00002Br 1  9D 45 03             sta ICBAH, x
00002Er 1               
00002Er 1  A9 04                lda #OPNIN              ; režim otevření: čtení (odpovídá druhému parametru příkazu OPEN v BASICu)
000030r 1  9D 4A 03             sta ICAX1, x
000033r 1  A9 00                lda #0                  ; odpovídá třetímu parametru příkazu OPEN v BASICu
000035r 1  9D 4B 03             sta ICAX2, x
000038r 1               
...
00003Cr 1               
00003Cr 1               ; ---------------------------------------------------------------------
00003Cr 1               ; čtení zprávy z kanálu číslo 4
00003Cr 1               ; ---------------------------------------------------------------------
00003Cr 1               .proc READ_MESSAGE_FROM_CHANNEL_4
00003Cr 1  A2 40                ldx #IOCB4              ; offset odvozený od čísla IOCB kanálu
00003Er 1  A9 07                lda #GETCHR             ; prováděná operace
000040r 1  9D 42 03             sta ICCOM, x            ; zápis prováděné operace do bloku IOCB
000043r 1               
000043r 1                       ; adresa bloku
000043r 1  A9 rr                lda #<buffer
000045r 1  9D 44 03             sta ICBAL, x
000048r 1  A9 rr                lda #>buffer
00004Ar 1  9D 45 03             sta ICBAH, x
00004Dr 1               
00004Dr 1                       ; nastavení počtu čtených bajtů
00004Dr 1  A9 34                lda #(end_of_buffer-buffer)
00004Fr 1  9D 48 03             sta ICBLL, x
000052r 1  A9 00                lda #0
000054r 1  9D 49 03             sta ICBLH, x
000057r 1               
...
00005Br 1               
00005Br 1               ; ---------------------------------------------------------------------
00005Br 1               ; zápis zprávy na obrazovku
00005Br 1               ; ---------------------------------------------------------------------
00005Br 1               .proc WRITE_MESSAGE_INTO_CHANNEL_0
00005Br 1  A2 00                ldx #IOCB0              ; offset odvozený od čísla IOCB kanálu
00005Dr 1  A9 0B                lda #PUTCHR             ; prováděná operace
00005Fr 1  9D 42 03             sta ICCOM, x            ; zápis prováděné operace do bloku IOCB
000062r 1               
000062r 1                       ; adresa bloku
000062r 1  A9 rr                lda #<buffer
000064r 1  9D 44 03             sta ICBAL, x
000067r 1  A9 rr                lda #>buffer
000069r 1  9D 45 03             sta ICBAH, x
00006Cr 1               
00006Cr 1                       ; nastavení počtu čtených bajtů
00006Cr 1  A9 34                lda #(end_of_buffer-buffer)
...
00007Ar 1               
00007Ar 1               
00007Ar 1               ; jméno zařízení (+ jméno souboru) ukončené nulou
00007Ar 1  48 3A 48 45  device_name: .byte "H:HELLO.TXT", 0
00007Er 1  4C 4C 4F 2E  
000082r 1  54 58 54 00  
000086r 1               
000086r 1               ; buffer
000086r 1  xx xx xx xx  buffer: .res 52
00008Ar 1  xx xx xx xx  
00008Er 1  xx xx xx xx  
000092r 1  xx xx xx xx  
000096r 1  xx xx xx xx  
00009Ar 1  xx xx xx xx  
00009Er 1  xx xx xx xx  
0000A2r 1  xx xx xx xx  
0000A6r 1  xx xx xx xx  
0000AAr 1  xx xx xx xx  
0000AEr 1  xx xx xx xx  
0000B2r 1  xx xx xx xx  
0000B6r 1  xx xx xx xx  
0000BAr 1               end_of_buffer:
0000BAr 1               
0000BAr 1               end:
0000BAr 1               
0000BAr 1               
0000BAr 1               .segment "EXEHDR"
