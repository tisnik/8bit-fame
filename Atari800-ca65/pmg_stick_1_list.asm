...
Main file   : pmg_stick_1.asm
Current file: pmg_stick_1.asm
...
000005r 1               
000005r 1  A9 64                lda #100                ; horizontální pozice druhého hráče
000007r 1  8D 01 D0             sta HPOSP1              ; uložit do řídicího registru HPOSP1 na čipu GTIA
00000Ar 1               
00000Ar 1  A9 78                lda #120                ; horizontální pozice třetího hráče
00000Cr 1  8D 02 D0             sta HPOSP2              ; uložit do řídicího registru HPOSP2 na čipu GTIA
00000Fr 1               
00000Fr 1  A9 8C                lda #140                ; horizontální pozice čtvrtého hráče
000011r 1  8D 03 D0             sta HPOSP3              ; uložit do řídicího registru HPOSP3 na čipu GTIA
000014r 1               
000014r 1  A9 CC                lda #HUE_GREEN<<4 + 12  ; barva prvního hráče (odstín+intenzita)
000016r 1  8D C0 02             sta PCOLR0              ; uložit do řídicího registru PCOLR0 na čipu GTIA
000019r 1               
000019r 1  A9 EC                lda #HUE_YELLOW<<4 + 12 ; barva druhého hráče (odstín+intenzita)
00001Br 1  8D C1 02             sta PCOLR1              ; uložit do řídicího registru PCOLR1 na čipu GTIA
00001Er 1               
00001Er 1  A9 5C                lda #HUE_MAGENTA<<4 + 12 ; barva třetího hráče (odstín+intenzita)
000020r 1  8D C2 02             sta PCOLR2              ; uložit do řídicího registru PCOLR2 na čipu GTIA
000023r 1               
000023r 1  A9 9C                lda #HUE_CYAN<<4 + 12   ; barva čtvrtého hráče (odstín+intenzita)
000025r 1  8D C3 02             sta PCOLR3              ; uložit do řídicího registru PCOLR3 na čipu GTIA
000028r 1               
000028r 1  A9 FF                lda #$ff                ; bitová maska všech hráčů
00002Ar 1  8D 0D D0             sta GRAFP0              ; uložit do řídicího registru GRAFP0 na čipu GTIA
00002Dr 1  8D 0E D0             sta GRAFP1              ; uložit do řídicího registru GRAFP1 na čipu GTIA
000030r 1  8D 0F D0             sta GRAFP2              ; uložit do řídicího registru GRAFP2 na čipu GTIA
000033r 1  8D 10 D0             sta GRAFP3              ; uložit do řídicího registru GRAFP3 na čipu GTIA
000036r 1               
000036r 1  A9 03                lda #3                  ; bitové pole: povolení hráčů i střel
000038r 1  8D 1D D0             sta GRACTL              ; uložit do řídicího registru GRACTL na čipu GTIA
00003Br 1               
00003Br 1  A9 00                lda #0                  ; priorita hráčů a pozadí
00003Dr 1  8D 6F 02             sta GPRIOR              ; uložit do řídicího registru GPRIOR na čipu GTIA
000040r 1               
000040r 1  A9 98                lda #152                ; paměťová stránka číslo 152
000042r 1  8D 07 D4             sta PMBASE
000045r 1               
000045r 1                       addr = 152*256
000045r 1  A2 08                ldx #8                  ; začneme na hodnotě o 1 vyšší
000047r 1               next_line:
000047r 1  BD rr rr             lda sprite-1, x         ; načíst
00004Ar 1  9D 32 9A             sta addr+PLAYER_0_OFFSET+50, x  ; uložit byte - první hráč
00004Dr 1  9D B2 9A             sta addr+PLAYER_1_OFFSET+50, x  ; uložit byte - druhý hráč
000050r 1  9D 32 9B             sta addr+PLAYER_2_OFFSET+50, x  ; uložit byte - třetí hráč
000053r 1  9D B2 9B             sta addr+PLAYER_3_OFFSET+50, x  ; uložit byte - čtvrtý hráč
000056r 1  CA                   dex                     ; snížit offset + nastavit příznaky
000057r 1  D0 EE                bne next_line           ; další byte spritu
000059r 1               
000059r 1  A9 2E                lda #46                 ; povolení PMG DMA
00005Br 1  8D 2F 02             sta SDMCTL
00005Er 1               
00005Er 1  A2 50                ldx #80                 ; výchozí pozice prvního hráče
000060r 1               loop:
000060r 1  8A                   txa                     ; uložení obsahu X do akumulátoru
000061r 1  20 rr rr             jsr _wait_vsync
000064r 1  20 rr rr             jsr _wait_vsync
000067r 1  AA                   tax                     ; obnovení obsahu X z akumulátoru
000068r 1               
000068r 1  AD 78 02             lda STICK0              ; čtení joysticku
00006Br 1  C9 0B                cmp #11                 ; je nakloněn doleva?
00006Dr 1  D0 01                bne not_left
00006Fr 1  CA                   dex                     ; posun hráče doleva
000070r 1               not_left:
000070r 1  C9 07                cmp #7                  ; je nakloněn doprava?
000072r 1  D0 01                bne not_right
000074r 1  E8                   inx                     ; posun hráče doprava
000075r 1               not_right:
000075r 1  8E 00 D0             stx HPOSP0              ; změna pozice prvního hráče
000078r 1  4C rr rr             jmp loop
00007Br 1               
00007Br 1               .endproc
00007Br 1               
00007Br 1               
00007Br 1               .proc   _wait_vsync
00007Br 1  A6 14                ldx     RTCLOK+2        ; čekání na konec snímku
00007Dr 1  E4 14        @wt:    cpx     RTCLOK+2
00007Fr 1  F0 FC                beq     @wt
000081r 1  60                   rts
000082r 1               .endproc
000082r 1               
000082r 1               
000082r 1               ; data
000082r 1  18 3C 7E DB  sprite:   .byte 24, 60, 126, 219, 255, 36, 90, 165
000086r 1  FF 24 5A A5  
00008Ar 1               
00008Ar 1               end:
00008Ar 1               
00008Ar 1               
00008Ar 1               .segment "EXEHDR"
