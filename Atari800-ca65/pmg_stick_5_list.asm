...
Main file   : pmg_stick_5.asm
Current file: pmg_stick_5.asm

000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; Ovládání PMG joystickem v horizontálním směru, změna barvy hráče stiskem
000000r 1               ; tlačítka joysticku.
000000r 1               ;
...
000041r 1               
000041r 1  A9 FF                lda #$ff                ; bitová maska všech hráčů
000043r 1  8D 0D D0             sta GRAFP0              ; uložit do řídicího registru GRAFP0 na čipu GTIA
000046r 1  8D 0E D0             sta GRAFP1              ; uložit do řídicího registru GRAFP1 na čipu GTIA
000049r 1  8D 0F D0             sta GRAFP2              ; uložit do řídicího registru GRAFP2 na čipu GTIA
00004Cr 1  8D 10 D0             sta GRAFP3              ; uložit do řídicího registru GRAFP3 na čipu GTIA
00004Fr 1               
00004Fr 1  A9 03                lda #3                  ; bitové pole: povolení hráčů i střel
000051r 1  8D 1D D0             sta GRACTL              ; uložit do řídicího registru GRACTL na čipu GTIA
000054r 1               
000054r 1  A9 10                lda #16                 ; priorita hráčů a pozadí -> povolení pátého hráče
000056r 1  8D 6F 02             sta GPRIOR              ; uložit do řídicího registru GPRIOR na čipu GTIA
000059r 1               
000059r 1  A9 98                lda #152                ; paměťová stránka číslo 152
00005Br 1  8D 07 D4             sta PMBASE
00005Er 1               
00005Er 1                       addr = 152*256
00005Er 1  A2 08                ldx #8                  ; začneme na hodnotě o 1 vyšší
000060r 1               next_line:
000060r 1  BD rr rr             lda sprite-1, x         ; načíst
000063r 1  9D 32 9A             sta addr+PLAYER_0_OFFSET+50, x  ; uložit byte - první hráč
000066r 1  9D B2 9A             sta addr+PLAYER_1_OFFSET+50, x  ; uložit byte - druhý hráč
000069r 1  9D 32 9B             sta addr+PLAYER_2_OFFSET+50, x  ; uložit byte - třetí hráč
00006Cr 1  9D B2 9B             sta addr+PLAYER_3_OFFSET+50, x  ; uložit byte - čtvrtý hráč
00006Fr 1  9D B2 99             sta addr+MISSILES_OFFSET+50, x  ; uložit byte - střely
000072r 1  CA                   dex                     ; snížit offset + nastavit příznaky
000073r 1  D0 EB                bne next_line           ; další byte spritu
000075r 1               
000075r 1  A9 2E                lda #46                 ; povolení PMG DMA
000077r 1  8D 2F 02             sta SDMCTL
00007Ar 1               
00007Ar 1  A0 0E                ldy #14                 ; barva prvního hráče
00007Cr 1  A2 50                ldx #80                 ; výchozí pozice prvního hráče
00007Er 1               loop:
00007Er 1  8A                   txa                     ; uložení obsahu X do akumulátoru
00007Fr 1  20 rr rr             jsr _wait_vsync
000082r 1  20 rr rr             jsr _wait_vsync
000085r 1  AA                   tax                     ; obnovení obsahu X z akumulátoru
000086r 1               
000086r 1  AD 78 02             lda STICK0              ; čtení joysticku
000089r 1  C9 0B                cmp #11                 ; je nakloněn doleva?
00008Br 1  D0 01                bne not_left
00008Dr 1  CA                   dex                     ; posun hráče doleva
00008Er 1               not_left:
00008Er 1  C9 07                cmp #7                  ; je nakloněn doprava?
000090r 1  D0 01                bne not_right
000092r 1  E8                   inx                     ; posun hráče doprava
000093r 1               not_right:
000093r 1  8E 00 D0             stx HPOSP0              ; změna pozice prvního hráče
000096r 1               
000096r 1  AD 84 02             lda STRIG0              ; 0=stisknuto (Z je nastaven)
000099r 1  D0 08                bne not_pressed         ; NE==NZ
00009Br 1               
00009Br 1  98                   tya
00009Cr 1  18                   clc
00009Dr 1  69 10                adc #16                 ; změna odstínu barvy, ne intenzity
00009Fr 1  A8                   tay
0000A0r 1  8C C0 02             sty PCOLR0              ; modifikovat barvový registr
0000A3r 1               not_pressed:
0000A3r 1               
0000A3r 1  4C rr rr            jmp loop
0000A6r 1               
0000A6r 1               .endproc
0000A6r 1               
0000A6r 1               
0000A6r 1               .proc   _wait_vsync
0000A6r 1  A6 14                ldx     RTCLOK+2        ; čekání na konec snímku
0000A8r 1  E4 14        @wt:    cpx     RTCLOK+2
0000AAr 1  F0 FC                beq     @wt
0000ACr 1  60                   rts
0000ADr 1               .endproc
0000ADr 1               
0000ADr 1               
0000ADr 1               ; data
0000ADr 1  18 3C 7E DB  sprite:   .byte 24, 60, 126, 219, 255, 36, 90, 165
0000B1r 1  FF 24 5A A5  
0000B5r 1               
0000B5r 1               end:
0000B5r 1               
0000B5r 1               
0000B5r 1               .segment "EXEHDR"
