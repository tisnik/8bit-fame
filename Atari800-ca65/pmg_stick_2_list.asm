...
Main file   : pmg_stick_2.asm
Current file: pmg_stick_2.asm

000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; Ovládání PMG joystickem v horizontálním směru, zobrazení čtyř střel.
000000r 1               ;
...
000019r 1               
000019r 1  A9 96                lda #150                ; horizontální pozice druhé střely
00001Br 1  8D 05 D0             sta HPOSM1              ; uložit do řídicího registru HPOSP1 na čipu GTIA
00001Er 1               
00001Er 1  A9 A0                lda #160                ; horizontální pozice třetí střely
000020r 1  8D 06 D0             sta HPOSM2              ; uložit do řídicího registru HPOSP2 na čipu GTIA
000023r 1               
000023r 1  A9 AA                lda #170                ; horizontální pozice čtvrté střely
...
00003Cr 1               
00003Cr 1  A9 FF                lda #$ff                ; bitová maska všech hráčů
00003Er 1  8D 0D D0             sta GRAFP0              ; uložit do řídicího registru GRAFP0 na čipu GTIA
000041r 1  8D 0E D0             sta GRAFP1              ; uložit do řídicího registru GRAFP1 na čipu GTIA
000044r 1  8D 0F D0             sta GRAFP2              ; uložit do řídicího registru GRAFP2 na čipu GTIA
000047r 1  8D 10 D0             sta GRAFP3              ; uložit do řídicího registru GRAFP3 na čipu GTIA
00004Ar 1               
00004Ar 1  A9 03                lda #3                  ; bitové pole: povolení hráčů i střel
00004Cr 1  8D 1D D0             sta GRACTL              ; uložit do řídicího registru GRACTL na čipu GTIA
00004Fr 1               
00004Fr 1  A9 00                lda #0                  ; priorita hráčů a pozadí
000051r 1  8D 6F 02             sta GPRIOR              ; uložit do řídicího registru GPRIOR na čipu GTIA
000054r 1               
000054r 1  A9 98                lda #152                ; paměťová stránka číslo 152
000056r 1  8D 07 D4             sta PMBASE
000059r 1               
000059r 1                       addr = 152*256
000059r 1  A2 08                ldx #8                  ; začneme na hodnotě o 1 vyšší
00005Br 1               next_line:
00005Br 1  BD rr rr             lda sprite-1, x         ; načíst
00005Er 1  9D 32 9A             sta addr+PLAYER_0_OFFSET+50, x  ; uložit byte - první hráč
000061r 1  9D B2 9A             sta addr+PLAYER_1_OFFSET+50, x  ; uložit byte - druhý hráč
000064r 1  9D 32 9B             sta addr+PLAYER_2_OFFSET+50, x  ; uložit byte - třetí hráč
000067r 1  9D B2 9B             sta addr+PLAYER_3_OFFSET+50, x  ; uložit byte - čtvrtý hráč
00006Ar 1  9D B2 99             sta addr+MISSILES_OFFSET+50, x  ; uložit byte - střely
00006Dr 1  CA                   dex                     ; snížit offset + nastavit příznaky
00006Er 1  D0 EB                bne next_line           ; další byte spritu
000070r 1               
000070r 1  A9 2E                lda #46                 ; povolení PMG DMA
000072r 1  8D 2F 02             sta SDMCTL
000075r 1               
000075r 1  A2 50                ldx #80                 ; výchozí pozice prvního hráče
000077r 1               loop:
000077r 1  8A                   txa                     ; uložení obsahu X do akumulátoru
000078r 1  20 rr rr             jsr _wait_vsync
00007Br 1  20 rr rr             jsr _wait_vsync
00007Er 1  AA                   tax                     ; obnovení obsahu X z akumulátoru
00007Fr 1               
00007Fr 1  AD 78 02             lda STICK0              ; čtení joysticku
000082r 1  C9 0B                cmp #11                 ; je nakloněn doleva?
000084r 1  D0 01                bne not_left
000086r 1  CA                   dex                     ; posun hráče doleva
000087r 1               not_left:
000087r 1  C9 07                cmp #7                  ; je nakloněn doprava?
000089r 1  D0 01                bne not_right
00008Br 1  E8                   inx                     ; posun hráče doprava
00008Cr 1               not_right:
00008Cr 1  8E 00 D0             stx HPOSP0              ; změna pozice prvního hráče
00008Fr 1  4C rr rr             jmp loop
000092r 1               
000092r 1               .endproc
000092r 1               
000092r 1               
000092r 1               .proc   _wait_vsync
000092r 1  A6 14                ldx     RTCLOK+2        ; čekání na konec snímku
000094r 1  E4 14        @wt:    cpx     RTCLOK+2
000096r 1  F0 FC                beq     @wt
000098r 1  60                   rts
000099r 1               .endproc
000099r 1               
000099r 1               
000099r 1               ; data
000099r 1  18 3C 7E DB  sprite:   .byte 24, 60, 126, 219, 255, 36, 90, 165
00009Dr 1  FF 24 5A A5  
0000A1r 1               
0000A1r 1               end:
0000A1r 1               
0000A1r 1               
