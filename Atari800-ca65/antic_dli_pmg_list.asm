...
00000Ar 1               
00000Ar 1  A9 50                lda #80                 ; horizontální pozice prvního hráče
00000Cr 1  8D 00 D0             sta HPOSP0              ; uložit do řídicího registru HPOSP0 na čipu GTIA
00000Fr 1               
00000Fr 1  A9 64                lda #100                ; horizontální pozice druhého hráče
000011r 1  8D 01 D0             sta HPOSP1              ; uložit do řídicího registru HPOSP1 na čipu GTIA
000014r 1               
000014r 1  A9 78                lda #120                ; horizontální pozice třetího hráče
000016r 1  8D 02 D0             sta HPOSP2              ; uložit do řídicího registru HPOSP2 na čipu GTIA
000019r 1               
000019r 1  A9 8C                lda #140                ; horizontální pozice čtvrtého hráče
00001Br 1  8D 03 D0             sta HPOSP3              ; uložit do řídicího registru HPOSP3 na čipu GTIA
00001Er 1               
00001Er 1  A9 CC                lda #HUE_GREEN<<4 + 12  ; barva prvního hráče (odstín+intenzita)
000020r 1  8D C0 02             sta PCOLR0              ; uložit do řídicího registru PCOLR0 na čipu GTIA
000023r 1               
000023r 1  A9 EC                lda #HUE_YELLOW<<4 + 12 ; barva druhého hráče (odstín+intenzita)
000025r 1  8D C1 02             sta PCOLR1              ; uložit do řídicího registru PCOLR1 na čipu GTIA
000028r 1               
000028r 1  A9 5C                lda #HUE_MAGENTA<<4 + 12 ; barva třetího hráče (odstín+intenzita)
00002Ar 1  8D C2 02             sta PCOLR2              ; uložit do řídicího registru PCOLR2 na čipu GTIA
00002Dr 1               
00002Dr 1  A9 9C                lda #HUE_CYAN<<4 + 12   ; barva čtvrtého hráče (odstín+intenzita)
00002Fr 1  8D C3 02             sta PCOLR3              ; uložit do řídicího registru PCOLR3 na čipu GTIA
000032r 1               
000032r 1  A9 FF                lda #$ff                ; bitová maska všech hráčů
000034r 1  8D 0D D0             sta GRAFP0              ; uložit do řídicího registru GRAFP0 na čipu GTIA
000037r 1  8D 0E D0             sta GRAFP1              ; uložit do řídicího registru GRAFP1 na čipu GTIA
00003Ar 1  8D 0F D0             sta GRAFP2              ; uložit do řídicího registru GRAFP2 na čipu GTIA
00003Dr 1  8D 10 D0             sta GRAFP3              ; uložit do řídicího registru GRAFP3 na čipu GTIA
000040r 1               
000040r 1  A9 03                lda #3                  ; bitové pole: povolení hráčů i střel
000042r 1  8D 1D D0             sta GRACTL              ; uložit do řídicího registru GRACTL na čipu GTIA
000045r 1               
000045r 1  A9 00                lda #0                  ; priorita hráčů a pozadí
000047r 1  8D 6F 02             sta GPRIOR              ; uložit do řídicího registru GPRIOR na čipu GTIA
00004Ar 1               
00004Ar 1  A9 98                lda #152                ; paměťová stránka číslo 152
00004Cr 1  8D 07 D4             sta PMBASE
00004Fr 1               
00004Fr 1                       addr = 152*256
00004Fr 1  A2 08                ldx #8                  ; začneme na hodnotě o 1 vyšší
000051r 1               next_line:
000051r 1  BD rr rr             lda sprite-1, x         ; načíst
000054r 1  9D 1E 9A             sta addr+PLAYER_0_OFFSET+30, x  ; uložit byte - první hráč
000057r 1  9D 9E 9A             sta addr+PLAYER_1_OFFSET+30, x  ; uložit byte - druhý hráč
00005Ar 1  9D 1E 9B             sta addr+PLAYER_2_OFFSET+30, x  ; uložit byte - třetí hráč
00005Dr 1  9D 9E 9B             sta addr+PLAYER_3_OFFSET+30, x  ; uložit byte - čtvrtý hráč
000060r 1  9D 50 9A             sta addr+PLAYER_0_OFFSET+80, x  ; uložit byte - první hráč
000063r 1  9D D0 9A             sta addr+PLAYER_1_OFFSET+80, x  ; uložit byte - druhý hráč
000066r 1  9D 50 9B             sta addr+PLAYER_2_OFFSET+80, x  ; uložit byte - třetí hráč
000069r 1  9D D0 9B             sta addr+PLAYER_3_OFFSET+80, x  ; uložit byte - čtvrtý hráč
00006Cr 1  CA                   dex                     ; snížit offset + nastavit příznaky
00006Dr 1  D0 E2                bne next_line           ; další byte spritu
00006Fr 1               
00006Fr 1  A9 2E                lda #46                 ; povolení PMG DMA
000071r 1  8D 2F 02             sta SDMCTL
000074r 1               
000074r 1                       ; vektor DLI
000074r 1  A9 rr                lda #<dli               ; nastavení DLI
000076r 1  8D 00 02             sta VDSLST
000079r 1  A9 rr                lda #>dli
00007Br 1  8D 01 02             sta VDSLST+1
00007Er 1               
00007Er 1                       ; povolení DLI
00007Er 1                       NMIEN_DLI=$80           ; maska povolení DLI
00007Er 1                       NMIEN_VBI=$40           ; maska povolení VBI
00007Er 1  A9 C0                lda #NMIEN_DLI | NMIEN_VBI ; povolení DLI
000080r 1  8D 0E D4             sta NMIEN
000083r 1               
000083r 1  4C rr rr     loop:   jmp loop
000086r 1               
000086r 1               
000086r 1               ; ---------------------------------------------------------------------
000086r 1               ; obsluha DLI
000086r 1               ; ---------------------------------------------------------------------
000086r 1               dli:
000086r 1  48                   pha                     ; uschovat akumulátor
000087r 1               
000087r 1  A9 2F                lda #$2f                ; dočasné barvy hráčů
000089r 1  8D 12 D0             sta COLPM0
00008Cr 1               
00008Cr 1  A9 4F                lda #$4f                ; dočasné barvy hráčů
00008Er 1  8D 13 D0             sta COLPM1
000091r 1               
000091r 1  A9 6F                lda #$6f                ; dočasné barvy hráčů
000093r 1  8D 14 D0             sta COLPM2
000096r 1               
000096r 1  A9 2F                lda #$2f                ; dočasné barvy hráčů
000098r 1  8D 15 D0             sta COLPM3
00009Br 1               
00009Br 1  AD rr rr             lda offset              ; horizontální pozice prvního hráče
00009Er 1  8D 00 D0             sta HPOSP0              ; uložit do řídicího registru HPOSP0 na čipu GTIA
0000A1r 1  C9 50                cmp #80                 ; prohazovat souřadnice 80 a 160
0000A3r 1  D0 07                bne next
0000A5r 1  A9 A0                lda #160
0000A7r 1  8D rr rr             sta offset
0000AAr 1               
0000AAr 1  68                   pla                     ; obnovit akumulátor
0000ABr 1  40                   rti                     ; návrat z DLI
0000ACr 1               next:
0000ACr 1  A9 50                lda #80
0000AEr 1  8D rr rr             sta offset
0000B1r 1               
0000B1r 1  68                   pla                     ; obnovit akumulátor
0000B2r 1  40                   rti                     ; návrat z DLI
0000B3r 1               
0000B3r 1               
0000B3r 1               .endproc
0000B3r 1               
...
0000B7r 1  rr rr        .byte <screen, >screen          ; počáteční adresa obrazové paměti
0000B9r 1  02 02 02 02  .res 10, DL_CHR40x8x1           ; opakovat řádky textového režimu 2 (GR.0)
0000BDr 1  02 02 02 02  
0000C1r 1  02 02        
0000C3r 1  82           .byte DL_DLI+DL_CHR40x8x1       ; GR.0 ovšem navíc s povolením DLI
0000C4r 1  02 02 02 02  .res 11, DL_CHR40x8x1           ; opakovat řádky textového režimu 2 (GR.0)
0000C8r 1  02 02 02 02  
0000CCr 1  02 02 02     
0000CFr 1  82           .byte DL_DLI+DL_CHR40x8x1       ; GR.0 ovšem navíc s povolením DLI
0000D0r 1  41 rr rr     .byte DL_JVB, <dlist, >dlist    ; skok na začátek display listu
0000D3r 1               
0000D3r 1  50           offset:   .byte 80
0000D4r 1               
0000D4r 1               ; data
0000D4r 1  18 3C 7E DB  sprite:   .byte 24, 60, 126, 219, 255, 36, 90, 165
0000D8r 1  FF 24 5A A5  
0000DCr 1               
0000DCr 1               end:
0000DCr 1               
0000DCr 1               .BSS
