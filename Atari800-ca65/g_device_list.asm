...
000000r 1               .CODE
000000r 1               
000000r 1               ; memory segment starts here
000000r 1               begin = $9800
000000r 1               
000000r 1               .org begin
009800  1               
009800  1               ; table containing vectors to subroutines implementing CIO operations
009800  1               ; addresses need to be decreased by one - subroutines are called via RTS instruction!
009800  1               hatabs_handler:
009800  1  27 98            .word g_open-1
009802  1  67 98            .word g_close-1
009804  1  72 98            .word g_get-1
009806  1  98 98            .word g_put-1
009808  1  64 98            .word g_nondef-1
00980A  1  64 98            .word g_nondef-1
00980C  1  4C 08 99         JMP g_init
00980F  1  00               .byte $00
009810  1               
009810  1               
009810  1               cursor_low_limit = $10
009810  1               cursor_top_limit = $70
009810  1               
009810  1               ; offsets that are updated during operations with devices
009810  1               
009810  1               ; read cursors for all four devices
009810  1               read_cursors:
009810  1  10 10 10 10      .byte cursor_low_limit, cursor_low_limit, cursor_low_limit, cursor_low_limit
009814  1               
009814  1               ; write cursors for all four devices
009814  1               write_cursors:
009814  1  10 10 10 10      .byte cursor_low_limit, cursor_low_limit, cursor_low_limit, cursor_low_limit
009818  1               
009818  1               ; colors assigned to all four sprites (one per device)
009818  1               dev_colors:
009818  1  1C 2C 6C AC      .byte $1C, $2C, $6C, $AC
00981C  1               
00981C  1               ; horizontal positions for all four sprites (one per device)
00981C  1               dev_hpos:
00981C  1  90 A4 B8 CC      .byte $90, $A4, $B8, $CC
009820  1               
009820  1               visual_markers:
009820  1  08 08 08 49      .byte $08, $08, $08, $49, $2A, $1C, $08, $FF
009824  1  2A 1C 08 FF  
009828  1               
009828  1               ; address of two bytes allocated on zero page:
009828  1               ; $CE, $CF    ; cur_base_l :L,:H
009828  1               cur_base_l = $ce
009828  1               cur_base_h = cur_base_l + 1
009828  1               ; the following address will be used in the code
009828  1               cur_base = cur_base_l
009828  1               
009828  1               
009828  1               ; ============
009828  1               ; HATABS_OPEN
009828  1               ; ============
009828  1               ; IN: ---
009828  1               ; OUT: Y: 01 = success
009828  1               ;        >80 = error
009828  1               
009828  1               ; check device number, we support G1 - G4 only
009828  1               g_open:
009828  1  A6 21                ldx ICDNOZ
00982A  1  CA                   dex           ; save devnum; normalize 1-4 to 0-3
00982B  1  8A                   txa
00982C  1  29 FC                and #$FC
00982E  1  F0 03                beq devnum_ok
009830  1  A0 82                ldy #NONDEV   ; error_code "no such device"
009832  1  60                   rts
009833  1               
009833  1               ; init the device in the command code, we support only (4,8,9,12,13)
009833  1               ; Binary Flags:
009833  1               ;   1    append (together with 8 only) => skip buffer cleanup and skip W cursor reset
009833  1               ;   4    reading => set read cursor, show device
009833  1               ;   8    writing => buffer cleanup, W cursor reset, show device
009833  1               ; any    some operation => success (LDY #SUCCES)
009833  1               
009833  1               devnum_ok:
009833  1  A0 84                ldy #NVALID   ; error_code "invalid command"
009835  1  A9 F2                lda #$F2
009837  1  24 2A                bit ICAX1Z
009839  1  D0 2C                bne open_exit ; throw the error
00983B  1  A9 08                lda #$08
00983D  1  24 2A                bit ICAX1Z
00983F  1  F0 16                beq test_read
009841  1               
009841  1  A9 01                lda #$01      ; test for append mode
009843  1  24 2A                bit ICAX1Z
009845  1  D0 0B                bne open_appnd
009847  1  20 BF 98             jsr get_dev_base
00984A  1  20 D3 98             jsr init_dev_data
00984D  1  A9 10                lda #cursor_low_limit
00984F  1  9D 14 98             sta write_cursors, x  ; reset write_cursor
009852  1               open_appnd:
009852  1  20 FB 98             jsr show_dev
009855  1  A0 01                ldy #SUCCES      ; writing => set success
009857  1               
009857  1               test_read:
009857  1  A9 04                lda #$04
009859  1  24 2A                bit ICAX1Z
00985B  1  F0 0A                beq open_exit
00985D  1  A9 10                lda #cursor_low_limit
00985F  1  9D 10 98             sta read_cursors, x  ; reset read_cursor
009862  1  20 FB 98             jsr show_dev
009865  1               g_nondef:
009865  1  A0 01                ldy #SUCCES
009867  1               open_exit:
009867  1  60                   rts
009868  1               
009868  1               
009868  1               ; ============
009868  1               ; HATABS_CLOSE
009868  1               ; ============
009868  1               ; IN: ---
009868  1               ; OUT: Y: 01 = success
009868  1               ;        >80 = error (see the CIO err_code list)
009868  1               
009868  1               g_close:
009868  1  A6 21                ldx ICDNOZ
00986A  1  CA                   dex
00986B  1  A9 00                lda #$00
00986D  1  9D 00 D0             sta HPOSP0, x    ; move sprite off the screen
009870  1  A0 01                ldy #SUCCES
009872  1  60                   rts
009873  1               
009873  1               ; ============
009873  1               ; HATABS_GET_CHAR
009873  1               ; ============
009873  1               ; IN:  X - channel number * $10
009873  1               ; OUT: Y: 01 = success
009873  1               ;        >80 = error (see the CIO err_code list)
009873  1               ;      A: the byte read
009873  1               g_get:
009873  1  BD 4A 03             lda ICAX1, x
009876  1  C9 09                cmp #$09
009878  1  D0 03                bne no_wr_append
00987A  1  A0 83                ldy #WRONLY   ; error_code "attempted to read a write-only device"
00987C  1  60                   rts
00987D  1               
00987D  1               no_wr_append:
00987D  1  BD 41 03             lda ICDNO, x
009880  1  AA                   tax
009881  1  CA                   dex           ; dev number 0-3 to X
009882  1  20 BF 98             jsr get_dev_base
009885  1  BD 10 98             lda read_cursors, x   ; read_cursor
009888  1  DD 14 98             cmp write_cursors, x  ; is lower then write_cursor?
00988B  1  90 03                bcc read_ok
00988D  1  A0 88                ldy #EOFERR      ; error_code "end of file"
00988F  1  60                   rts
009890  1               
009890  1               read_ok:
009890  1  A8                   tay
009891  1  B1 CE                lda (cur_base), y     ; read byte from the device
009893  1  FE 10 98             inc read_cursors, x
009896  1  A0 01                ldy #SUCCES
009898  1  60                   rts
009899  1               
009899  1               ; ============
009899  1               ; HATABS_PUT_CHAR
009899  1               ; ============
009899  1               ; IN:  A: the byte to write
009899  1               ; OUT: Y: 01 = success
009899  1               ;        >80 = error (see the CIO err_code list)
009899  1               g_put:
009899  1  48                   pha
00989A  1  A0 92                ldy #FNCNOT   ; error_code "function not implemented in handler"
00989C  1  A9 08                lda #$08
00989E  1  24 2A                bit ICAX1Z    ; throw the error
0098A0  1  F0 11                beq exit_w_error
0098A2  1  BD 41 03             lda ICDNO, x
0098A5  1  AA                   tax
0098A6  1  CA                   dex                        ; dev number 0-3 to X
0098A7  1  20 BF 98             jsr get_dev_base
0098AA  1  BD 14 98             lda write_cursors, x
0098AD  1  C9 70                cmp #cursor_top_limit      ; still a space in the buff?
0098AF  1  90 04                bcc write_ok
0098B1  1  A0 88                ldy #EOFERR   ; error_code "end of file"
0098B3  1               exit_w_error:
0098B3  1  68                   pla
0098B4  1  60                   rts
0098B5  1               
0098B5  1               write_ok:
0098B5  1  A8                   tay
0098B6  1  68                   pla
0098B7  1  91 CE                sta (cur_base),y          ; store the byte in the device
0098B9  1  FE 14 98             inc write_cursors, x
0098BC  1  A0 01                ldy #SUCCES
0098BE  1  60                   rts
0098BF  1               
0098BF  1               
0098BF  1               ; ============
0098BF  1               ; Get Device Base Address
0098BF  1               ; ============
0098BF  1               ; IN:   X: device num (0-3)
0098BF  1               ; OUT:  $CE, $CF: current device base :L,:H
0098BF  1               get_dev_base:
0098BF  1  A9 9A                lda #>begin+2      ; PMBASE + 2
0098C1  1  85 CF                sta cur_base_h
0098C3  1  A0 00                ldy #$00
0098C5  1  8A                   txa
0098C6  1  4A                   lsr
0098C7  1  90 02                bcc dev02
0098C9  1  A0 80                ldy #$80
0098CB  1               dev02:
0098CB  1  84 CE                sty cur_base_l
0098CD  1  4A                   lsr
0098CE  1  90 02                bcc dev01
0098D0  1  E6 CF                inc cur_base_h
0098D2  1               dev01:
0098D2  1  60                   rts
0098D3  1               
0098D3  1               ; ============
0098D3  1               ; Init Device Data
0098D3  1               ; ============
0098D3  1               ; IN:   cur_base must be set
0098D3  1               ; OUT:  device data cleared + markers drawn
0098D3  1               init_dev_data:
0098D3  1  A0 80                ldy #$80
0098D5  1  A9 00                lda #$00
0098D7  1               null_loop:
0098D7  1  88                   dey
0098D8  1  91 CE                sta (cur_base),y
0098DA  1  D0 FB                bne null_loop
0098DC  1  8A                   txa
0098DD  1  48                   pha
0098DE  1  A2 07                ldx #$07	; draw visual markers - top
0098E0  1  A0 0F                ldy #$0F
0098E2  1               marker_bot:
0098E2  1  BD 20 98             lda visual_markers, x
0098E5  1  91 CE                sta (cur_base),y
0098E7  1  88                   dey
0098E8  1  CA                   dex
0098E9  1  D0 F7                bne marker_bot
0098EB  1               
0098EB  1  A2 07                ldx #$07	; draw visual markers - bottom
0098ED  1  A0 70                ldy #$70
0098EF  1               marker_top:
0098EF  1  BD 20 98             lda visual_markers, x
0098F2  1  91 CE                sta (cur_base),y
0098F4  1  C8                   iny
0098F5  1  CA                   dex
0098F6  1  D0 F7                bne marker_top
0098F8  1               
0098F8  1  68                   pla
0098F9  1  AA                   tax
0098FA  1  60                   rts
0098FB  1               
0098FB  1               ; ============
0098FB  1               ; Show Device on the Screen
0098FB  1               ; ============
0098FB  1               ; IN:   X: device num (0-3)
0098FB  1               ; OUT:  ---
0098FB  1               ; Show Device on the Screen
0098FB  1               show_dev:
0098FB  1  BD 18 98             lda dev_colors, x
0098FE  1  9D C0 02             sta PCOLR0, x
009901  1  BD 1C 98             lda dev_hpos, x
009904  1  9D 00 D0             sta HPOSP0, x
009907  1  60                   rts
009908  1               
009908  1               ; ============
009908  1               ; main()
009908  1               ; ============
009908  1               ; !!! contains extra PLA at the end
009908  1               ;     to be called from BASIC using USR()
009908  1               g_init:
009908  1  48                   pha
009909  1  8A                   txa
00990A  1  48                   pha
...
00990D  1               
00990D  1               
00990D  1               ; Add device using OS method
00990D  1               
00990D  1  A2 47                ldx #'G'      ; device name
00990F  1  A0 00                ldy #<hatabs_handler
009911  1  A9 98                lda #>hatabs_handler
009913  1  20 86 E4             jsr PHENTV    ; ADD_NEWDEV
009916  1  B0 35                bcs exit_init
009918  1               
009918  1               ; set space in RAM and init GTIA
009918  1               
009918  1  A9 00                lda #<begin
00991A  1  8D E5 02             sta MEMTOP
00991D  1  A9 98                lda #>begin
00991F  1  8D E6 02             sta MEMTOP+1
009922  1               
009922  1  8D 07 D4             sta PMBASE
009925  1  A2 03                ldx #$03          ; init all devices data
009927  1               loop_init:
009927  1  20 BF 98             jsr get_dev_base
00992A  1  20 D3 98             jsr init_dev_data
00992D  1  CA                   dex
00992E  1  10 F7                bpl loop_init
009930  1               
009930  1  A9 00                lda #$00
009932  1  8D 00 D0             sta HPOSP0
009935  1  8D 01 D0             sta HPOSP1
009938  1  8D 02 D0             sta HPOSP2
00993B  1  8D 03 D0             sta HPOSP3
00993E  1  A9 01                lda #$01
009940  1  8D 6F 02             sta GPRIOR
009943  1  A9 03                lda #$03    ; enable sprites
009945  1  8D 1D D0             sta GRACTL
009948  1  A9 2E                lda #$2E    ; enable DMA for display list and sprites, set normal playfield width
00994A  1  8D 2F 02             sta SDMCTL
00994D  1               exit_init:
00994D  1  68                   pla
00994E  1  A8                   tay
00994F  1  68                   pla
...
009954  1               end:
009954  1               
009954  1               .segment "EXEHDR"
009954  1  FF FF        .word   $ffff                   ; uvodni sekvence bajtu v souboru XEX
009956  1  00 98        .word   begin                   ; zacatek kodoveho segmentu
009958  1  53 99        .word   end-1                   ; konec kodoveho segmentu
00995A  1               
00995A  1               ; finito
00995A  1               
