...
Main file   : gtia_mode_7.asm
Current file: gtia_mode_7.asm

000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; Display list s mnoha různými textovými režimy.
000000r 1               ; Postupná změna režimu na GTIA 9, 11 a 10.
000000r 1               ; ---------------------------------------------------------------------
000000r 1               
000000r 1               
...
00000Cr 1  A9 00                lda #0                  ; kód vypisovaného znaku
00000Er 1               fill_screen:
00000Er 1  99 rr rr             sta screen, y           ; tisk znaku na zvolené místo na obrazovce
000011r 1  18                   clc
...
000017r 1  D0 F5                bne fill_screen          ; skok, pokud Y>40*6
000019r 1               
000019r 1  A9 CC                lda #HUE_GREEN<<4 + 12  ; barva prvního hráče (odstín+intenzita)
00001Br 1  8D C0 02             sta PCOLR0              ; uložit do řídicího registru PCOLR0 na čipu GTIA
00001Er 1               
00001Er 1  A9 EC                lda #HUE_YELLOW<<4 + 12 ; barva druhého hráče (odstín+intenzita)
000020r 1  8D C1 02             sta PCOLR1              ; uložit do řídicího registru PCOLR1 na čipu GTIA
000023r 1               
000023r 1  A9 5C                lda #HUE_MAGENTA<<4 + 12 ; barva třetího hráče (odstín+intenzita)
000025r 1  8D C2 02             sta PCOLR2              ; uložit do řídicího registru PCOLR2 na čipu GTIA
000028r 1               
000028r 1  A9 9C                lda #HUE_CYAN<<4 + 12   ; barva čtvrtého hráče (odstín+intenzita)
00002Ar 1  8D C3 02             sta PCOLR3              ; uložit do řídicího registru PCOLR3 na čipu GTIA
00002Dr 1               
00002Dr 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
000030r 1               
000030r 1  AD 6F 02             lda GPRIOR              ; modifikace stínového registru GPRIOR
000033r 1  09 40                ora  #$40               ; výběr GTIA režimu GR.9
000035r 1  8D 6F 02             sta GPRIOR
000038r 1               
000038r 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
00003Br 1               
00003Br 1  A9 0F                lda #15                 ; intenzita barev
00003Dr 1  8D C8 02             sta COLOR4              ; ulozit nový kód barvy do registru COLOR2
000040r 1               
000040r 1  AD 6F 02             lda GPRIOR              ; modifikace stínového registru GPRIOR
000043r 1  09 C0                ora  #$c0               ; výběr GTIA režimu GR.11
000045r 1  8D 6F 02             sta GPRIOR
000048r 1               
000048r 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
00004Br 1               
00004Br 1  A9 CC                lda #HUE_GREEN<<4 + 12  ; barva prvního hráče (odstín+intenzita)
00004Dr 1  8D C0 02             sta PCOLR0              ; uložit do řídicího registru PCOLR0 na čipu GTIA
000050r 1               
000050r 1  A9 EC                lda #HUE_YELLOW<<4 + 12 ; barva druhého hráče (odstín+intenzita)
000052r 1  8D C1 02             sta PCOLR1              ; uložit do řídicího registru PCOLR1 na čipu GTIA
000055r 1               
000055r 1  A9 5C                lda #HUE_MAGENTA<<4 + 12 ; barva třetího hráče (odstín+intenzita)
000057r 1  8D C2 02             sta PCOLR2              ; uložit do řídicího registru PCOLR2 na čipu GTIA
00005Ar 1               
00005Ar 1  A9 9C                lda #HUE_CYAN<<4 + 12   ; barva čtvrtého hráče (odstín+intenzita)
00005Cr 1  8D C3 02             sta PCOLR3              ; uložit do řídicího registru PCOLR3 na čipu GTIA
00005Fr 1               
00005Fr 1  AD 6F 02             lda GPRIOR              ; modifikace stínového registru GPRIOR
000062r 1  29 3F        	and  #%00111111
000064r 1  09 80                ora  #$80               ; výběr GTIA režimu GR.10
000066r 1  8D 6F 02             sta GPRIOR
000069r 1               
000069r 1  4C rr rr     loop:   jmp loop
00006Cr 1               .endproc
00006Cr 1               
00006Cr 1               
00006Cr 1               ; ---------------------------------------------------------------------
00006Cr 1               ; čekání na stisk klávesy
00006Cr 1               ; ---------------------------------------------------------------------
00006Cr 1               .proc get_key
00006Cr 1                       KBHANDLER = $e424       ; rutina pro cteni klavesy
00006Cr 1  AD 25 E4             lda KBHANDLER+1         ; cteni horni casti adresy ulozene v ROM
00006Fr 1  48                   pha                     ; ulozeni na zasobnik
000070r 1  AD 24 E4             lda KBHANDLER           ; cteni dolni casti adresy ulozene v ROM
000073r 1  48                   pha                     ; ulozeni na zasobnik
000074r 1  60                   rts                     ; vyber adresy ze zasobniku + skok
000075r 1                                               ; zde neni nutne mit RTS
000075r 1               .endproc
000075r 1               
000075r 1               
000075r 1               dlist:
000075r 1  70 70 70     .byte DL_BLK8, DL_BLK8, DL_BLK8 ; 3x8=24 prázdných obrazových řádků
000078r 1  47           .byte DL_LMS+DL_CHR20x16x2      ; určení počáteční adresy obrazové paměti + jeden řádek režimu 7
000079r 1  rr rr        .byte <screen, >screen          ; počáteční adresa obrazové paměti
00007Br 1  07           .byte DL_CHR20x16x2             ; jeden řádek textového režimu 7 (GR.2)
00007Cr 1  02           .byte DL_CHR40x8x1              ; jeden řádek textového režimu 2 (GR.0)
00007Dr 1  06           .byte DL_CHR20x8x2              ; jeden řádek textového režimu 6 (GR.1)
00007Er 1  02           .byte DL_CHR40x8x1              ; jeden řádek textového režimu 2 (GR.0)
00007Fr 1  07           .byte DL_CHR20x16x2             ; jeden řádek textového režimu 7 (GR.2)
000080r 1  07           .byte DL_CHR20x16x2             ; jeden řádek textového režimu 7 (GR.2)
000081r 1  08           .byte DL_MAP40x8x4              ; jeden řádek grafického režimu 8 (GR.3)
000082r 1  08           .byte DL_MAP40x8x4              ; jeden řádek grafického režimu 8 (GR.3)
000083r 1  07           .byte DL_CHR20x16x2             ; jeden řádek textového režimu 7 (GR.2)
000084r 1  07           .byte DL_CHR20x16x2             ; jeden řádek textového režimu 7 (GR.2)
000085r 1  07           .byte DL_CHR20x16x2             ; jeden řádek textového režimu 7 (GR.2)
000086r 1  02           .byte DL_CHR40x8x1              ; jeden řádek textového režimu 2 (GR.0)
000087r 1  41 rr rr     .byte DL_JVB, <dlist, >dlist    ; skok na začátek display listu
00008Ar 1               
00008Ar 1               end:
00008Ar 1               
00008Ar 1               .BSS
