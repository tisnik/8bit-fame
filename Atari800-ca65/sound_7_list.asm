...
Main file   : sound_7.asm
Current file: sound_7.asm
...
000000r 1               .CODE
000000r 1               
000000r 1               ; Bit 7 Bit 6  Bit 5   Výsledek
000000r 1               ; 0     0      0       17 bit poly → 5 bit poly
000000r 1               ; 0     0      1       5 bit poly
000000r 1               ; 0     1      0       4 bit poly → 5 bit poly
000000r 1               ; 0     1      1       5 bit poly
000000r 1               ; 1     0      0       17 bit poly
000000r 1               ; 1     0      1       čistý tón
000000r 1               ; 1     1      0       4 bit poly
000000r 1               ; 1     1      1       čistý tón
000000r 1               
000000r 1               PURE_TONE_MASK      = %10100000
000000r 1               POLY_4_BIT_MASK     = %11000000
000000r 1               POLY_5_BIT_MASK     = %01100000
000000r 1               POLY_17_BIT_MASK    = %10000000
000000r 1               POLY_4_5_BIT_MASK   = %01000000
000000r 1               POLY_17_5_BIT_MASK  = %00000000
000000r 1               
000000r 1               ; Frekvence CPU (PAL) = 1.7734470 MHz:
000000r 1               ;     f_clk = 1773447 Hz
000000r 1               ;
000000r 1               ; Pokud je AUDCTL.0 nastaven na nulu, dělí se 28:
000000r 1               ;     f_in = f_clk / 28 = 1773447 / 28 = 63337 Hz
000000r 1               ;
000000r 1               ; Pokud je AUDCTL.7 == 1 and AUDCTL.5 == 1, dělí se f_in dvěma
000000r 1               ;     f_in_2 = f_in / 2 = 63337 / 2 = 31688 Hz
000000r 1               ;
000000r 1               ; Tato frekvence se přímo dělí obsahem registru AUDF1 (+1), takže pro #71:
000000r 1               ;     f_out = f_in_2 / (71+1) = 31688 / 72 = 439 Hz
000000r 1               ;
000000r 1               ; S přimhouřením obou uší tedy dostaneme komorní A (440 Hz)
000000r 1               
000000r 1               .proc main
000000r 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
000003r 1               
000003r 1  A9 00                lda #00                 ; základní tón odvozený od 64kHz
000005r 1  8D 08 D2             sta AUDCTL              ; zápis do řídicího registru čipu POKEY
000008r 1               
000008r 1  A9 47                lda #71                 ; dělič základní frekvence
00000Ar 1  8D 00 D2             sta AUDF1               ; zápis do registru děliče frekvence pro zvukový kanál číslo 1 čipu POKEY
00000Dr 1               
00000Dr 1               next:
00000Dr 1  A9 AA                lda #PURE_TONE_MASK+10  ; hlasitost (0-15)
00000Fr 1  8D 01 D2             sta AUDC1               ; zápis do registru řízení hlasitosti pro zvukový kanál číslo 1 čipu POKEY
000012r 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
000015r 1               
000015r 1  A9 CA                lda #POLY_4_BIT_MASK+10 ; hlasitost (0-15) + noise
000017r 1  8D 01 D2             sta AUDC1               ; zápis do registru řízení hlasitosti pro zvukový kanál číslo 1 čipu POKEY
00001Ar 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
00001Dr 1               
00001Dr 1  A9 6A                lda #POLY_5_BIT_MASK+10 ; hlasitost (0-15) + noise
00001Fr 1  8D 01 D2             sta AUDC1               ; zápis do registru řízení hlasitosti pro zvukový kanál číslo 1 čipu POKEY
000022r 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
000025r 1               
000025r 1  A9 8A                lda #POLY_17_BIT_MASK+10 ; hlasitost (0-15) + noise
000027r 1  8D 01 D2             sta AUDC1               ; zápis do registru řízení hlasitosti pro zvukový kanál číslo 1 čipu POKEY
00002Ar 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
00002Dr 1               
00002Dr 1  A9 4A                lda #POLY_4_5_BIT_MASK+10 ; hlasitost (0-15) + noise
00002Fr 1  8D 01 D2             sta AUDC1               ; zápis do registru řízení hlasitosti pro zvukový kanál číslo 1 čipu POKEY
000032r 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
000035r 1               
000035r 1  A9 0A                lda #POLY_17_5_BIT_MASK+10 ; hlasitost (0-15) + noise
000037r 1  8D 01 D2             sta AUDC1               ; zápis do registru řízení hlasitosti pro zvukový kanál číslo 1 čipu POKEY
00003Ar 1  20 rr rr             jsr get_key             ; čekání na stisk klávesy
00003Dr 1               
00003Dr 1  A9 01                lda #1                  ; dělič základní frekvence (nejvyssi frekvence)
00003Fr 1  8D 00 D2             sta AUDF1               ; zápis do registru děliče frekvence pro zvukový kanál číslo 1 čipu POKEY
000042r 1  4C rr rr             jmp next
000045r 1               
000045r 1  A9 00                lda #0                  ; vypnutí zvuku (nulová hlasitost)
000047r 1  8D 01 D2             sta AUDC1               ; zápis do registru řízení hlasitosti pro zvukový kanál číslo 1 čipu POKEY
00004Ar 1               
00004Ar 1  4C rr rr     loop:   jmp loop
00004Dr 1               .endproc
00004Dr 1               
00004Dr 1               
00004Dr 1               ; ---------------------------------------------------------------------
00004Dr 1               ; čekání na stisk klávesy
00004Dr 1               ; ---------------------------------------------------------------------
00004Dr 1               .proc get_key
00004Dr 1                       KBHANDLER = $e424       ; rutina pro cteni klavesy
00004Dr 1  AD 25 E4             lda KBHANDLER+1         ; cteni horni casti adresy ulozene v ROM
000050r 1  48                   pha                     ; ulozeni na zasobnik
000051r 1  AD 24 E4             lda KBHANDLER           ; cteni dolni casti adresy ulozene v ROM
000054r 1  48                   pha                     ; ulozeni na zasobnik
000055r 1  60                   rts                     ; vyber adresy ze zasobniku + skok
000056r 1                                               ; zde neni nutne mit RTS
000056r 1               .endproc
000056r 1               
000056r 1               
000056r 1               end:
000056r 1               
000056r 1               .segment "EXEHDR"
