...
Main file   : antic_vbi_dli_2.asm
Current file: antic_vbi_dli_2.asm
...
000005r 1  A9 rr                lda #>dlist             ; vyšší byte adresy display listu
000007r 1  8D 31 02             sta SDLSTH
00000Ar 1               
00000Ar 1  A0 00                ldy #0                  ; počitadlo zápisů
00000Cr 1               clear:
00000Cr 1  98                   tya                     ; kód vypisovaného znaku
00000Dr 1  99 rr rr             sta screen, y           ; tisk znaku na zvolené místo na obrazovce
000010r 1  99 rr rr             sta screen+40*8, y
000013r 1  99 rr rr             sta screen+40*16, y
000016r 1  C8                   iny                     ; zvětšit hodnotu počitadla a offsetu
000017r 1  C0 80                cpy #$80
000019r 1  D0 F1                bne clear               ; skok
00001Br 1               
00001Br 1                       ; vektor DLI
00001Br 1  A9 rr                lda #<dli               ; nastavení DLI
00001Dr 1  8D 00 02             sta VDSLST
000020r 1  A9 rr                lda #>dli
000022r 1  8D 01 02             sta VDSLST+1
000025r 1               
000025r 1  A9 50                lda #80                 ; horizontální pozice prvního hráče
000027r 1  8D 00 D0             sta HPOSP0              ; uložit do řídicího registru HPOSP0 na čipu GTIA
00002Ar 1               
00002Ar 1  A9 64                lda #100                ; horizontální pozice druhého hráče
00002Cr 1  8D 01 D0             sta HPOSP1              ; uložit do řídicího registru HPOSP1 na čipu GTIA
00002Fr 1               
00002Fr 1  A9 78                lda #120                ; horizontální pozice třetího hráče
000031r 1  8D 02 D0             sta HPOSP2              ; uložit do řídicího registru HPOSP2 na čipu GTIA
000034r 1               
000034r 1  A9 8C                lda #140                ; horizontální pozice čtvrtého hráče
000036r 1  8D 03 D0             sta HPOSP3              ; uložit do řídicího registru HPOSP3 na čipu GTIA
000039r 1               
000039r 1  A9 CC                lda #HUE_GREEN<<4 + 12  ; barva prvního hráče (odstín+intenzita)
00003Br 1  8D C0 02             sta PCOLR0              ; uložit do řídicího registru PCOLR0 na čipu GTIA
00003Er 1               
00003Er 1  A9 EC                lda #HUE_YELLOW<<4 + 12 ; barva druhého hráče (odstín+intenzita)
000040r 1  8D C1 02             sta PCOLR1              ; uložit do řídicího registru PCOLR1 na čipu GTIA
000043r 1               
000043r 1  A9 5C                lda #HUE_MAGENTA<<4 + 12 ; barva třetího hráče (odstín+intenzita)
000045r 1  8D C2 02             sta PCOLR2              ; uložit do řídicího registru PCOLR2 na čipu GTIA
000048r 1               
000048r 1  A9 9C                lda #HUE_CYAN<<4 + 12   ; barva čtvrtého hráče (odstín+intenzita)
00004Ar 1  8D C3 02             sta PCOLR3              ; uložit do řídicího registru PCOLR3 na čipu GTIA
00004Dr 1               
00004Dr 1  A9 FF                lda #$ff                ; bitová maska všech hráčů
00004Fr 1  8D 0D D0             sta GRAFP0              ; uložit do řídicího registru GRAFP0 na čipu GTIA
000052r 1  8D 0E D0             sta GRAFP1              ; uložit do řídicího registru GRAFP1 na čipu GTIA
000055r 1  8D 0F D0             sta GRAFP2              ; uložit do řídicího registru GRAFP2 na čipu GTIA
000058r 1  8D 10 D0             sta GRAFP3              ; uložit do řídicího registru GRAFP3 na čipu GTIA
00005Br 1               
00005Br 1  A9 03                lda #3                  ; bitové pole: povolení hráčů i střel
00005Dr 1  8D 1D D0             sta GRACTL              ; uložit do řídicího registru GRACTL na čipu GTIA
000060r 1               
000060r 1  A9 01                lda #1                   ; priorita hráčů a pozadí
000062r 1  8D 6F 02             sta GPRIOR              ; uložit do řídicího registru GPRIOR na čipu GTIA
000065r 1               
000065r 1  A9 98                lda #152                ; paměťová stránka číslo 152
000067r 1  8D 07 D4             sta PMBASE
00006Ar 1               
00006Ar 1                       addr = 152*256
00006Ar 1  A2 08                ldx #8                  ; začneme na hodnotě o 1 vyšší
00006Cr 1               next_line:
00006Cr 1  BD rr rr             lda sprite-1, x         ; načíst
00006Fr 1  9D 64 9A             sta addr+PLAYER_0_OFFSET+100, x  ; uložit byte - první hráč
000072r 1  9D E4 9A             sta addr+PLAYER_1_OFFSET+100, x  ; uložit byte - druhý hráč
000075r 1  9D 64 9B             sta addr+PLAYER_2_OFFSET+100, x  ; uložit byte - třetí hráč
000078r 1  9D E4 9B             sta addr+PLAYER_3_OFFSET+100, x  ; uložit byte - čtvrtý hráč
00007Br 1  CA                   dex                     ; snížit offset + nastavit příznaky
00007Cr 1  D0 EE                bne next_line           ; další byte spritu
00007Er 1               
00007Er 1  A9 2E                lda #46                 ; povolení PMG DMA
000080r 1  8D 2F 02             sta SDMCTL
000083r 1               
000083r 1                       ; nastavit vektor pro odloženou VBI
000083r 1  A9 07                lda #7                  ; změna vektoru pro odložené VBI
000085r 1  A2 rr                ldx #>horizontal_movement
000087r 1  A0 rr                ldy #<horizontal_movement
000089r 1  20 5C E4             jsr SETVBV              ; zavolat službu systému pro nastavení vektoru
00008Cr 1               
00008Cr 1                       ; povolení DLI
00008Cr 1                       NMIEN_DLI=$80           ; maska povolení DLI
00008Cr 1                       NMIEN_VBI=$40           ; maska povolení VBI
00008Cr 1  A9 C0                lda #NMIEN_DLI | NMIEN_VBI ; povolení DLI
00008Er 1  8D 0E D4             sta NMIEN
000091r 1               
000091r 1               loop:
000091r 1  4C rr rr             jmp loop                ; program vlastně nice nedělá - jen cyklí!
000094r 1               .endproc
000094r 1               
000094r 1               
000094r 1               ; ---------------------------------------------------------------------
...
000094r 1               ; ---------------------------------------------------------------------
000094r 1               .proc   horizontal_movement
