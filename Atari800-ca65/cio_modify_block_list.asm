...
Main file   : cio_modify_block.asm
Current file: cio_modify_block.asm
...
000000r 1               
000000r 1               .proc main
000000r 1  A2 00                ldx #0                  ; offset odvozený od čísla IOCB kanálu (nutno násobit šestnácti)
000002r 1  A9 07                lda #GETCHR             ; prováděná operace
000004r 1  9D 42 03             sta ICCOM, x
000007r 1               
000007r 1                       ; adresa, na který se blok zapíše
000007r 1  A9 rr                lda #<buffer
000009r 1  9D 44 03             sta ICBAL, x
00000Cr 1  A9 rr                lda #>buffer
00000Er 1  9D 45 03             sta ICBAH, x
000011r 1               
000011r 1                       ; nastavení počtu čtených bajtů
000011r 1  A9 0A                lda #(end_of_buffer-buffer)
000013r 1  9D 48 03             sta ICBLL, x
000016r 1  A9 00                lda #0
000018r 1  9D 49 03             sta ICBLH, x
00001Br 1               
00001Br 1                       ; zavolání rutiny pro CIO
00001Br 1  20 56 E4             jsr CIOV
00001Er 1               
00001Er 1                       ; modifikace obsahu bufferu
00001Er 1  A2 00                ldx #0
000020r 1               next_char:
000020r 1  BD rr rr     	lda buffer, x
000023r 1  49 80        	eor #%10000000          ; inverse video
000025r 1  9D rr rr     	sta buffer, x
000028r 1  E8           	inx
000029r 1  E0 0A        	cpx #(end_of_buffer-buffer)
00002Br 1  D0 F3        	bne next_char
00002Dr 1               
00002Dr 1  A2 00                ldx #0                  ; offset odvozený od čísla IOCB kanálu (nutno násobit šestnácti)
00002Fr 1  A9 0B                lda #PUTCHR             ; prováděná operace
000031r 1  9D 42 03             sta ICCOM, x
000034r 1               
000034r 1                       ; adresa zapisovaného bloku
000034r 1  A9 rr                lda #<buffer
000036r 1  9D 44 03             sta ICBAL, x
000039r 1  A9 rr                lda #>buffer
00003Br 1  9D 45 03             sta ICBAH, x
00003Er 1               
00003Er 1                       ; nastavení počtu zapisovaných bajtů
00003Er 1  A9 0A                lda #(end_of_buffer-buffer)
000040r 1  9D 48 03             sta ICBLL, x
000043r 1  A9 00                lda #0
000045r 1  9D 49 03             sta ICBLH, x
000048r 1               
000048r 1                       ; zavolání rutiny pro CIO
000048r 1  20 56 E4             jsr CIOV
00004Br 1               loop:
00004Br 1  4C rr rr             jmp loop                ; program vlastně nice nedělá - jen cyklí!
00004Er 1               .endproc
00004Er 1               
00004Er 1  xx xx xx xx  buffer: .res 10
000052r 1  xx xx xx xx  
000056r 1  xx xx        
000058r 1               end_of_buffer:
000058r 1               
000058r 1               end:
000058r 1               
000058r 1               
000058r 1               .segment "EXEHDR"
