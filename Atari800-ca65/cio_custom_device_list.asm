...
Main file   : cio_custom_device.asm
Current file: cio_custom_device.asm
...
000000r 1               .CODE
000000r 1               
000000r 1               IOCB4 = $40              ; offset odvozený od čísla IOCB kanálu (nutno násobit šestnácti)
000000r 1               
000000r 1               
000000r 1               
000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; hlavní program
000000r 1               ; ---------------------------------------------------------------------
000000r 1               .proc main
...
000012r 1               loop:
000012r 1  4C rr rr             jmp loop                ; program vlastně nice nedělá - jen cyklí!
000015r 1               .endproc
000015r 1               
000015r 1               
000015r 1               
000015r 1               ; ---------------------------------------------------------------------
000015r 1               ; registrace nového zařízení
000015r 1               ; ---------------------------------------------------------------------
000015r 1               .proc REGISTER_NULL_DEVICE
000015r 1               
000015r 1               insert:
000015r 1  A2 00                ldx #0
000017r 1               next_place:
000017r 1  BD 1A 03             lda HATABS, x
00001Ar 1  F0 07                beq empty_place
00001Cr 1  E8                   inx
00001Dr 1  E8                   inx
00001Er 1  E8                   inx
00001Fr 1  D0 F6                bne next_place
000021r 1  F0 F2                beq insert
000023r 1               empty_place:
000023r 1  A9 4E                lda #'N'
000025r 1  9D 1A 03             sta HATABS, x
000028r 1  A9 rr                lda #<null_table
00002Ar 1  9D 1B 03             sta HATABS+1, x
00002Dr 1  A9 rr                lda #>null_table
00002Fr 1  9D 1C 03             sta HATABS+2, x
000032r 1  60                   rts                     ; návrat ze subrutiny
000033r 1               .endproc
000033r 1               
000033r 1               
000033r 1               
...
000037r 1               
000037r 1               ; ---------------------------------------------------------------------
000037r 1               ; uzavření kanálu číslo 4
000037r 1               ; ---------------------------------------------------------------------
000037r 1               .proc CLOSE_CHANNEL_4
000037r 1  A2 40                ldx #IOCB4              ; offset odvozený od čísla IOCB kanálu
000039r 1  A9 0C                lda #CLOSE              ; prováděná operace
00003Br 1  9D 42 03             sta ICCOM, x            ; zápis prováděné operace do bloku IOCB
00003Er 1  20 56 E4             jsr CIOV                ; zavolání rutiny pro CIO
000041r 1  60                   rts                     ; návrat ze subrutiny
000042r 1               .endproc
000042r 1               
000042r 1               
000042r 1               
...
000049r 1               
000049r 1                       ; specifikace jména zařízení + jména souboru (16bitový ukazatel)
000049r 1  A9 rr                lda #<device_name
00004Br 1  9D 44 03             sta ICBAL, x
00004Er 1  A9 rr                lda #>device_name
000050r 1  9D 45 03             sta ICBAH, x
000053r 1               
000053r 1  A9 08                lda #OPNOT              ; režim otevření: zápis (odpovídá druhému parametru příkazu OPEN v BASICu)
000055r 1  9D 4A 03             sta ICAX1, x
000058r 1  A9 00                lda #0                  ; odpovídá třetímu parametru příkazu OPEN v BASICu
00005Ar 1  9D 4B 03             sta ICAX2, x
00005Dr 1               
00005Dr 1  20 56 E4             jsr CIOV                ; zavolání rutiny pro CIO
000060r 1               
000060r 1  60                   rts                     ; návrat ze subrutiny
000061r 1               .endproc
000061r 1               
000061r 1               
000061r 1               
000061r 1               ; ---------------------------------------------------------------------
000061r 1               ; zápis jednoho znaku do kanálu číslo 4
000061r 1               ; ---------------------------------------------------------------------
000061r 1               .proc WRITE_BYTE_CHANNEL_4
000061r 1  A2 40                ldx #IOCB4              ; offset odvozený od čísla IOCB kanálu
000063r 1  A9 0B                lda #PUTCHR             ; prováděná operace
000065r 1  9D 42 03             sta ICCOM, x            ; zápis prováděné operace do bloku IOCB
000068r 1               
000068r 1                       ; adresa zapisovaného bloku
000068r 1  A9 rr                lda #<char_to_put
00006Ar 1  9D 44 03             sta ICBAL, x
00006Dr 1  A9 rr                lda #>char_to_put
00006Fr 1  9D 45 03             sta ICBAH, x
000072r 1               
000072r 1                       ; nastavení počtu zapisovaných bajtů
000072r 1  A9 01                lda #1
000074r 1  9D 48 03             sta ICBLL, x
000077r 1  A9 00                lda #0
000079r 1  9D 49 03             sta ICBLH, x
00007Cr 1               
00007Cr 1  20 56 E4             jsr CIOV                ; zavolání rutiny pro CIO
00007Fr 1  60                   rts                     ; návrat ze subrutiny
000080r 1               .endproc
000080r 1               
000080r 1               
000080r 1               
000080r 1               ; ---------------------------------------------------------------------
000080r 1               ; kontrola chyby při zápisu
000080r 1               ; ---------------------------------------------------------------------
000080r 1               .proc CHECK_ERROR
000080r 1  A2 40                ldx #IOCB4              ; offset odvozený od čísla IOCB kanálu
000082r 1               
000082r 1  BD 43 03             lda ICSTA, x            ; status poslední operace
000085r 1  10 06                bpl ok                  ; test bitu 7
000087r 1               
000087r 1  A9 12                lda #18                 ; kod barvy
000089r 1  8D C6 02             sta COLOR2              ; ulozit do registru COLOR2
00008Cr 1  60                   rts                     ; návrat ze subrutiny
00008Dr 1               ok:
00008Dr 1  A9 C3                lda #$c3                ; kod barvy
00008Fr 1  8D C6 02             sta COLOR2              ; ulozit do registru COLOR2
000092r 1  60                   rts                     ; návrat ze subrutiny
000093r 1               .endproc
000093r 1               
000093r 1               
000093r 1               
000093r 1               ; ---------------------------------------------------------------------
000093r 1               ; subrutiny pro realizaci operací zařízení
000093r 1               ; ---------------------------------------------------------------------
000093r 1               .proc OPEN_OPERATION
000093r 1  A9 6F                lda #'o'                ; ATASCII hodnota znaku
000095r 1  A0 01                ldy #1                  ; nastavit registr Y
000097r 1  91 58                sta (88),y              ; tisk znaku na obrazovku
000099r 1  A9 01                lda #1                  ; chybový kód
00009Br 1  A8                   tay                     ; musí být uložen jak v A, tak i v Y
00009Cr 1  60           	rts
00009Dr 1               .endproc
00009Dr 1               
00009Dr 1               
...
0000A1r 1  91 58                sta (88),y              ; tisk znaku na obrazovku
0000A3r 1  A9 01                lda #1                  ; chybový kód
...
0000A9r 1  A0 51                ldy #81                 ; nastavit registr Y
0000ABr 1  91 58                sta (88),y              ; tisk znaku na obrazovku
0000ADr 1  A9 01                lda #1                  ; chybový kód
0000AFr 1  A8                   tay                     ; musí být uložen jak v A, tak i v Y
0000B0r 1  60           	rts
0000B1r 1               .endproc
0000B1r 1               
0000B1r 1               
...
0000B5r 1  91 58                sta (88),y              ; tisk znaku na obrazovku
0000B7r 1  A9 01                lda #1                  ; chybový kód
...
0000BDr 1  A0 A1                ldy #161                ; nastavit registr Y
0000BFr 1  91 58                sta (88),y              ; tisk znaku na obrazovku
0000C1r 1  A9 01                lda #1                  ; chybový kód
0000C3r 1  A8                   tay                     ; musí být uložen jak v A, tak i v Y
0000C4r 1  60           	rts
0000C5r 1               .endproc
0000C5r 1               
0000C5r 1               
