...
Main file   : sound_F.asm
Current file: sound_F.asm

000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; Generování zvuků čipem POKEY.
000000r 1               ;
000000r 1               ; Přímé přehrávání samplů.
000000r 1               ; Použit je jen zvukový kanál číslo 1.
000000r 1               ; ---------------------------------------------------------------------
000000r 1               
...
000003r 1               
000003r 1  A9 00                lda #00                 ; základní tón odvozený od 64kHz + základní nastavení čtyř zvukových kanálů
000005r 1  8D 08 D2             sta AUDCTL              ; zápis do řídicího registru čipu POKEY
000008r 1               
000008r 1  20 rr rr             jsr disable_all_interrupts
00000Br 1               
00000Br 1  A2 00                ldx #0                  ; vynulovat počitadlo
00000Dr 1               loop:
00000Dr 1  BD rr rr             lda delays, x           ; načtení doby trvání
000010r 1  8D rr rr             sta msc
000013r 1               l0:
000013r 1  BD rr rr             lda volumes, x          ; načtení amplitudy
000016r 1  8D 01 D2             sta AUDC1               ; přímý zápis amplitudy (včetně bitu číslo 5)
000019r 1  AC rr rr             ldy tempo
00001Cr 1               l1:
00001Cr 1  88                   dey                     ; krátká zpožďovací smyčka
00001Dr 1  D0 FD                bne l1
00001Fr 1  CE rr rr             dec msc
000022r 1  D0 EF                bne l0                  ; vnější zpožďovací smyčka
000024r 1               
000024r 1  E8                   inx                     ; nový sampl
000025r 1  EC rr rr             cpx nc                  ; jsme na konci dat?
000028r 1  D0 E3                bne loop                ; zatím ne - pokračujeme v přehrávání
00002Ar 1               
00002Ar 1  A2 00                ldx #0                  ; začneme od začátku
00002Cr 1  F0 DF                beq loop                ; když už je příznak nastavený, proč ho nevyužít
00002Er 1               .endproc
00002Er 1               
00002Er 1               
00002Er 1               
00002Er 1               ; ---------------------------------------------------------------------
00002Er 1               ; přehrávací smyčka nesmí být rušena žádnými dalšími podprogramy
00002Er 1               ; ---------------------------------------------------------------------
00002Er 1               .proc disable_all_interrupts
00002Er 1  A9 03                lda #3                  ; povolení práce s klávesnicí, zákaz serial I/O
000030r 1  8D 0F D2             sta SKCTL               ; zápis do řídicího registru čipu POKEY
000033r 1               
000033r 1  A9 00                lda #00
000035r 1  8D 0E D4             sta NMIEN               ; zákaz DLI i VBI
000038r 1  8D 00 D4             sta DMACTL              ; zákaz zobrazování
00003Br 1  8D 0E D2             sta IRQEN               ; zákaz nemaskovatelného přerušení
00003Er 1               
00003Er 1  60                   rts                     ; návrat z podprogramu
00003Fr 1               .endproc
00003Fr 1               
00003Fr 1               
00003Fr 1               
00003Fr 1               ; ---------------------------------------------------------------------
00003Fr 1               ; čekání na stisk klávesy
00003Fr 1               ; ---------------------------------------------------------------------
00003Fr 1               .proc get_key
00003Fr 1                       KBHANDLER = $e424       ; rutina pro cteni klavesy
00003Fr 1  AD 25 E4             lda KBHANDLER+1         ; cteni horni casti adresy ulozene v ROM
000042r 1  48                   pha                     ; ulozeni na zasobnik
000043r 1  AD 24 E4             lda KBHANDLER           ; cteni dolni casti adresy ulozene v ROM
000046r 1  48                   pha                     ; ulozeni na zasobnik
000047r 1  60                   rts                     ; vyber adresy ze zasobniku + skok
000048r 1                                               ; zde neni nutne mit RTS
000048r 1               .endproc
000048r 1               
000048r 1               
000048r 1               ; čítač samplů
000048r 1               nc:
000048r 1  1C           .byte 28
000049r 1               
000049r 1               ; tabulka amplitud
000049r 1               volumes:
000049r 1  18 19 1A 1B  .byte 24, 25, 26, 27, 28, 30, 31
00004Dr 1  1C 1E 1F     
000050r 1  1E 1D 1C 1B  .byte 30, 29, 28, 27, 26, 25, 24
000054r 1  1A 19 18     
000057r 1  17 16 15 14  .byte 23, 22, 21, 20, 19, 18, 17
00005Br 1  13 12 11     
00005Er 1  12 13 14 15  .byte 18, 19, 20, 21, 22, 23, 24
000062r 1  16 17 18     
000065r 1               
000065r 1               ; tabulka doby trvání amplitud
000065r 1               delays:
000065r 1  01 01 01 02  .byte 1, 1, 1, 2, 2, 2, 3
000069r 1  02 02 03     
00006Cr 1  06 03 02 02  .byte 6, 3, 2, 2, 2, 1, 1
000070r 1  02 01 01     
000073r 1  01 01 01 02  .byte 1, 1, 1, 2, 2, 2, 3
000077r 1  02 02 03     
00007Ar 1  06 03 02 02  .byte 6, 3, 2, 2, 2, 1, 1
00007Er 1  02 01 01     
000081r 1               
000081r 1               ; vyšší hodnota - nižší tón
000081r 1               tempo:
000081r 1  01           .byte 1
000082r 1               
000082r 1               ; čítač vnější zpožďovací smyčky
000082r 1               msc:
000082r 1  00           .byte 0
000083r 1               
000083r 1               end:
000083r 1               
000083r 1               .segment "EXEHDR"
