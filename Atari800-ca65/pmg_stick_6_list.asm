...
Main file   : pmg_stick_6.asm
Current file: pmg_stick_6.asm

000000r 1               ; ---------------------------------------------------------------------
000000r 1               ; Ovládání PMG joystickem v horizontálním i vertikálním směru.
000000r 1               ;
...
00001Er 1               
00001Er 1  A9 8E                lda #142                ; horizontální pozice třetí střely
000020r 1  8D 06 D0             sta HPOSM2              ; uložit do řídicího registru HPOSP2 na čipu GTIA
000023r 1               
000023r 1  A9 8C                lda #140                ; horizontální pozice čtvrté střely
...
000046r 1               
000046r 1  A9 10                lda #16                 ; priorita hráčů a pozadí -> povolení pátého hráče
...
000055r 1  9D 32 9A             sta addr+PLAYER_0_OFFSET+50, x  ; uložit byte - první hráč
...
000061r 1  9D B2 99             sta addr+MISSILES_OFFSET+50, x  ; uložit byte - střely
000064r 1  CA                   dex                     ; snížit offset + nastavit příznaky
000065r 1  D0 EB                bne next_line           ; další byte spritu
000067r 1               
000067r 1  A9 2E                lda #46                 ; povolení PMG DMA
000069r 1  8D 2F 02             sta SDMCTL
00006Cr 1               
00006Cr 1               ; zde začíná "herní kernel"
00006Cr 1  A2 50                ldx #80                 ; výchozí pozice prvního hráče
00006Er 1  A0 3C                ldy #60                 ; X i Y
000070r 1               loop:
000070r 1  20 rr rr             jsr _wait_vsync         ; čekání na vykreslení snímku
000073r 1  20 rr rr             jsr _wait_vsync
000076r 1               
000076r 1  AD 78 02             lda STICK0              ; čtení joysticku
000079r 1  C9 0B                cmp #11                 ; je nakloněn doleva?
00007Br 1  D0 01                bne not_left
00007Dr 1  CA                   dex                     ; posun hráče doleva
00007Er 1               not_left:
00007Er 1  C9 07                cmp #7                  ; je nakloněn doprava?
000080r 1  D0 01                bne not_right
000082r 1  E8                   inx                     ; posun hráče doprava
000083r 1               not_right:
000083r 1  C9 0E                cmp #14                 ; je nakloněn nahoru?
000085r 1  D0 04                bne not_up
000087r 1  88                   dey                     ; změna Y-ové souřadnice hráče
000088r 1  20 rr rr             jsr draw_at_y           ; překreslení spritu
00008Br 1               not_up:
00008Br 1  C9 0D                cmp #13                 ; je nakloněn dolů?
00008Dr 1  D0 04                bne not_down
00008Fr 1  C8                   iny                     ; změna Y-ové souřadnice hráče
000090r 1  20 rr rr             jsr draw_at_y           ; překreslení spritu
000093r 1               not_down:
000093r 1  8E 00 D0             stx HPOSP0              ; změna pozice prvního hráče
000096r 1  4C rr rr             jmp loop
000099r 1               
000099r 1               .endproc
000099r 1               
000099r 1               
000099r 1               .proc   _wait_vsync
000099r 1  8A                   txa                     ; uložení obsahu X do akumulátoru
00009Ar 1  A6 14                ldx     RTCLOK+2        ; čekání na konec snímku
00009Cr 1  E4 14        @wt:    cpx     RTCLOK+2
00009Er 1  F0 FC                beq     @wt
0000A0r 1  AA                   tax                     ; obnovení obsahu X z akumulátoru
0000A1r 1  60                   rts
0000A2r 1               .endproc
0000A2r 1               
0000A2r 1               
0000A2r 1               .proc  draw_at_y
0000A2r 1                       addr = 152*256          ; adresa první stránky se sprity
0000A2r 1  8A                   txa
0000A3r 1  48                   pha                     ; uložit X na zásobník
0000A4r 1  98                   tya
0000A5r 1  48                   pha                     ; uložit Y na zásobník
0000A6r 1               
0000A6r 1  A2 0A                ldx #10                 ; začneme na hodnotě o 1 vyšší
0000A8r 1               next_line:
0000A8r 1  BD rr rr             lda sprite-1, x         ; načíst
0000ABr 1  99 00 9A             sta addr+PLAYER_0_OFFSET, y  ; uložit byte - první hráč
0000AEr 1  88                   dey
0000AFr 1  CA                   dex                     ; snížit offset + nastavit příznaky
0000B0r 1  D0 F6                bne next_line           ; další byte spritu
0000B2r 1               
0000B2r 1  68                   pla
0000B3r 1  A8                   tay                     ; obnovit Y ze zásobníku
0000B4r 1  68                   pla
0000B5r 1  AA                   tax                     ; obnovit X ze zásobníku
0000B6r 1  60                   rts                     ; návrat z podprogramu
0000B7r 1               .endproc
0000B7r 1               
0000B7r 1               
0000B7r 1               ; data
0000B7r 1  00 18 3C 7E  sprite:   .byte 0, 24, 60, 126, 219, 255, 36, 90, 165, 0
0000BBr 1  DB FF 24 5A  
0000BFr 1  A5 00        
0000C1r 1               
0000C1r 1               end:
0000C1r 1               
0000C1r 1               
0000C1r 1               .segment "EXEHDR"
