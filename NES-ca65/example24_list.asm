...
Main file   : example24.asm
Current file: example24.asm
...
000043r 1               
000043r 1  A2 00 BD 02          flip_bit_block ATTRS, 8, 4, %01000000
000047r 1  02 49 40 9D  
...
0000DAr 1  8E 00 20             stx PPUCTRL             ; nastavení PPUCTRL = 0 (NMI)
0000DDr 1  8E 01 20             stx PPUMASK             ; nastavení PPUMASK = 0
0000E0r 1  8E 10 40             stx DMC_FREQ            ; zákaz DMC IRQ
0000E3r 1               
0000E3r 1  A2 40                ldx #$40
0000E5r 1  8E 17 40             stx $4017               ; interrupt inhibit bit
0000E8r 1               
0000E8r 1                       ; čekání na vnitřní inicializaci PPU (dva snímky)
0000E8r 1  2C 02 20 10          wait_for_frame
0000ECr 1  FB           
0000EDr 1  2C 02 20 10          wait_for_frame
0000F1r 1  FB           
0000F2r 1               
0000F2r 1                       ; vymazání obsahu RAM
0000F2r 1  A9 00 95 00          clear_ram
0000F6r 1  9D 00 01 9D  
0000FAr 1  00 02 9D 00  
0000FEr 1  03 9D 00 04  
000102r 1  9D 00 05 9D  
000106r 1  00 06 9D 00  
00010Ar 1  07 E8 D0 E6  
00010Er 1               
00010Er 1                       ; čekání na další snímek
00010Er 1  2C 02 20 10          wait_for_frame
000112r 1  FB           
000113r 1               
000113r 1                       ; nastavení barvové palety
000113r 1  20 rr rr             jsr load_palette  ; zavolání subrutiny
000116r 1               
000116r 1                       ; nastavení spritů
000116r 1  20 rr rr             jsr load_sprites  ; zavolání subrutiny
000119r 1               
000119r 1  A9 0A        	lda #10           ; inicializace čítačů
00011Br 1  85 FE        	sta COUNTER1
00011Dr 1  85 FF        	sta COUNTER2
00011Fr 1               
00011Fr 1                       ; vlastní herní smyčka je prozatím prázdná
00011Fr 1               game_loop:
00011Fr 1  4C rr rr             jmp game_loop           ; nekonečná smyčka (později rozšíříme)
000122r 1               .endproc
000122r 1               
000122r 1               
000122r 1               
000122r 1               ; vynulování barvové palety
000122r 1               .proc clear_palette
000122r 1  AD 02 20 A9          ppu_data_palette_address
000126r 1  3F 8D 06 20  
00012Ar 1  A9 00 8D 06  
00012Er 1  20           
00012Fr 1               
00012Fr 1  A2 20                ldx #$20        ; počitadlo barev v paletě: 16+16
000131r 1  A9 00                lda #$00        ; vynulování každé barvy
000133r 1               
000133r 1               :
000133r 1  8D 07 20             sta PPUDATA     ; zápis barvy
000136r 1  CA                   dex             ; snížení hodnoty počitadla
000137r 1  D0 FA                bne :-
000139r 1               
000139r 1  60                   rts             ; návrat ze subrutiny
00013Ar 1               .endproc
00013Ar 1               
00013Ar 1               
00013Ar 1               
00013Ar 1               ; nastavení barvové palety
00013Ar 1               .proc load_palette
00013Ar 1  AD 02 20 A9          ppu_data_palette_address
00013Er 1  3F 8D 06 20  
000142r 1  A9 00 8D 06  
000146r 1  20           
000147r 1               
000147r 1                       ; $3f00-$3f0f - paleta pozadí
000147r 1                       ; $3f10-$3f1f - paleta spritů
000147r 1               
000147r 1  A2 00                ldx #$00        ; vynulovat počitadlo a offset
000149r 1               
000149r 1               :
000149r 1  BD rr rr             lda palette, x  ; načíst bajt s offsetem
00014Cr 1  8D 07 20             sta PPUDATA     ; zápis barvy do PPU
00014Fr 1  E8                   inx             ; zvýšit počitadlo/offset
000150r 1  E0 20                cpx #32         ; limit počtu barev
000152r 1  D0 F5                bne :-          ; opakovat smyčku 32x
000154r 1               
000154r 1  60                   rts             ; návrat ze subrutiny
000155r 1               .endproc
000155r 1               
000155r 1               
000155r 1               
000155r 1               ; načtení spritů
000155r 1               .proc load_sprites
000155r 1  A2 00                ldx #0
000157r 1               :
000157r 1  BD rr rr             lda spritedata,X  ; budeme přesouvat data z této oblasti
00015Ar 1  9D 00 02             sta $0200,X       ; uložení do paměti spritů
00015Dr 1  E8                   inx               ; zvýšení hodnoty počitadla
00015Er 1  E0 20                cpx #32           ; každý sprite má 4 bajty: y-coord, tile, attributy, y-coord * 8 spritů = 32
000160r 1  D0 F5                bne :-
000162r 1               
000162r 1  58                   cli               ; vynulování bitu I - povolení přerušení
000163r 1  A9 80                lda #%10000000
000165r 1  8D 00 20             sta PPUCTRL       ; při každém VBLANK se vyvolá NMI (důležité!)
000168r 1               
000168r 1  A9 10                lda #%00010000    ; povolení zobrazení spritů
00016Ar 1  8D 01 20             sta PPUMASK
00016Dr 1               
00016Dr 1  60                   rts               ; návrat ze subrutiny
00016Er 1               .endproc
00016Er 1               
00016Er 1               
00016Er 1               
00016Er 1               ; samotná barvová paleta
00016Er 1               palette:
00016Er 1  22 29 1A 0F      .byte $22, $29, $1a, $0F, $22, $36, $17, $0F, $22, $30, $21, $0F, $22, $27, $17, $0F  ; barvy pozadí
000172r 1  22 36 17 0F  
000176r 1  22 30 21 0F  
00017Ar 1  22 27 17 0F  
00017Er 1  22 16 27 18      .byte $22, $16, $27, $18, $22, $1A, $30, $27, $22, $16, $30, $27, $22, $0F, $36, $17  ; barvy spritů
000182r 1  22 1A 30 27  
000186r 1  22 16 30 27  
00018Ar 1  22 0F 36 17  
00018Er 1               
00018Er 1               ; data pro větší množství spritů
00018Er 1               spritedata:
00018Er 1  10 00 00 08      .byte $10, $00, $00, $08   ; y-coord, tile number, attributes, x-coord
000192r 1  10 01 00 10      .byte $10, $01, $00, $10
000196r 1  18 02 00 08      .byte $18, $02, $00, $08
00019Ar 1  18 03 00 10      .byte $18, $03, $00, $10
00019Er 1  20 04 00 08      .byte $20, $04, $00, $08
0001A2r 1  20 05 00 10      .byte $20, $05, $00, $10
0001A6r 1  28 06 00 08      .byte $28, $06, $00, $08
0001AAr 1  28 07 00 10      .byte $28, $07, $00, $10
0001AEr 1               
0001AEr 1               
0001AEr 1               
0001AEr 1               ; ---------------------------------------------------------------------
0001AEr 1               ; Tabulka vektorů CPU
0001AEr 1               ; ---------------------------------------------------------------------
0001AEr 1               
0001AEr 1               .segment "VECTORS"
