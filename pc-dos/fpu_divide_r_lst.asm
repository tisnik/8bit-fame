     1                                  ;-----------------------------------------------------------------------------
     2                                  ; Zakladni aritmeticke operace provadena matematickym koprocesorem: deleni
     3                                  ; s otocenymi operandy.
     4                                  ;
     5                                  ; Tento demonstracni priklad je pouzity v serialu o programovani
     6                                  ; grafickych dem a her na PC v DOSu:
     7                                  ; https://www.root.cz/serialy/vyvoj-her-a-grafickych-dem-pro-platformu-pc/
     8                                  ;
     9                                  ;-----------------------------------------------------------------------------
    10                                  
    11                                  org  0x100        ; zacatek kodu pro programy typu COM (vzdy se zacina na 256)
    12                                  
    13                                  start:
    14 00000000 EB5C                            jmp main             ; skok na zacatek kodu
    15                                  
    16                                  %include "io.asm"            ; nacist symboly, makra a podprogramy
     1                              <1> ;-----------------------------------------------------------------------------
    ...
    25                              <1> 
    26                              <1> %endif
    17                                  %include "print.asm"         ; nacist symboly, makra a podprogramy
     1                              <1> ;-----------------------------------------------------------------------------
   ...
   127                              <1> %endif
    ...
    25                                          print_float32_as_hex ; zobrazeni FP hodnoty v hexadecimalnim tvaru
    43 00000068 D91E[5200]          <1>  fstp dword [float32]
    44 0000006C 66A1[5200]          <1>  mov eax, [float32]
    45                              <1>  print_hex eax
    23 00000070 60                  <2>  pusha
    24 00000071 6689C2              <2>  mov edx, %1
    25 00000074 66BB[3A000000]      <2>  mov ebx, hex_message
    26 0000007A E885FF              <2>  call hex2string
    27                              <2>  print_string hex_message
    15 0000007D BA[3A00]            <3>  mov dx, %1
    16 00000080 B409                <3>  mov ah, 9
    17 00000082 CD21                <3>  int 0x21
    28 00000084 61                  <2>  popa
    26                                  
    27                                          wait_key             ; cekani na klavesu
    22 00000085 31C0                <1>  xor ax, ax
    23 00000087 CD16                <1>  int 0x16
    28                                          exit                 ; navrat do DOSu
    16 00000089 B44C                <1>  mov ah, 0x4c
    17 0000008B CD21                <1>  int 0x21
